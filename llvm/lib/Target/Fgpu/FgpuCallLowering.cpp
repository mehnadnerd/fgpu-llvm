...
FgpuCallLowering::FgpuCallLowering(const FgpuTargetLowering &TLI)
    : CallLowering(&TLI) {}

struct FgpuOutgoingValueAssigner : public CallLowering::OutgoingValueAssigner {
  ...
  FgpuOutgoingValueAssigner(CCAssignFn *AssignFn_, const char *Func,
                            bool IsReturn)
  ...
  bool assignArg(unsigned ValNo, EVT OrigVT, MVT ValVT, MVT LocVT,
    ...
    FgpuCCState &State = static_cast<FgpuCCState &>(State_);
...
struct FgpuIncomingValueAssigner : public CallLowering::IncomingValueAssigner {
  /// This is the name of the function being called
  /// FIXME: Relying on this is unsound
  const char *Func = nullptr;
  ...
  FgpuIncomingValueAssigner(CCAssignFn *AssignFn_, const char *Func,
                            bool IsReturn)
...
namespace {
class FgpuIncomingValueHandler : public CallLowering::IncomingValueHandler {
  const FgpuSubtarget &STI;

public:
  FgpuIncomingValueHandler(MachineIRBuilder &MIRBuilder,
                           MachineRegisterInfo &MRI)
      : IncomingValueHandler(MIRBuilder, MRI),
        STI(MIRBuilder.getMF().getSubtarget<FgpuSubtarget>()) {}
...
class CallReturnHandler : public FgpuIncomingValueHandler {
public:
  CallReturnHandler(MachineIRBuilder &MIRBuilder, MachineRegisterInfo &MRI,
                    MachineInstrBuilder &MIB)
      : FgpuIncomingValueHandler(MIRBuilder, MRI), MIB(MIB) {}
...
void FgpuIncomingValueHandler::assignValueToReg(Register ValVReg,
                                                Register PhysReg,
                                                CCValAssign &VA) {
...
Register FgpuIncomingValueHandler::getStackAddress(uint64_t Size,
                                                   int64_t Offset,
                                                   MachinePointerInfo &MPO,
...
void FgpuIncomingValueHandler::assignValueToAddress(Register ValVReg,
...
unsigned
FgpuIncomingValueHandler::assignCustomValue(CallLowering::ArgInfo &Arg,
...
namespace {
class FgpuOutgoingValueHandler : public CallLowering::OutgoingValueHandler {
  const FgpuSubtarget &STI;

public:
  FgpuOutgoingValueHandler(MachineIRBuilder &MIRBuilder,
                           MachineRegisterInfo &MRI, MachineInstrBuilder &MIB)
      : OutgoingValueHandler(MIRBuilder, MRI),
        STI(MIRBuilder.getMF().getSubtarget<FgpuSubtarget>()), MIB(MIB) {}
...
void FgpuOutgoingValueHandler::assignValueToReg(Register ValVReg,
                                                Register PhysReg,
                                                CCValAssign &VA) {
...
Register FgpuOutgoingValueHandler::getStackAddress(uint64_t Size,
                                                   int64_t Offset,
                                                   MachinePointerInfo &MPO,
                                                   ISD::ArgFlagsTy Flags) {
  MachineFunction &MF = MIRBuilder.getMF();
  ...
  auto SPReg = MIRBuilder.buildCopy(p0, Register(Fgpu::SP));
...
void FgpuOutgoingValueHandler::assignValueToAddress(Register ValVReg,
                                                    Register Addr, LLT MemTy,
                                                    MachinePointerInfo &MPO,
                                                    CCValAssign &VA) {
  MachineFunction &MF = MIRBuilder.getMF();
...
unsigned
FgpuOutgoingValueHandler::assignCustomValue(CallLowering::ArgInfo &Arg,
                                            ArrayRef<CCValAssign> VAs) {
  const CCValAssign &VALo = VAs[0];
  const CCValAssign &VAHi = VAs[1];
...
bool FgpuCallLowering::lowerReturn(MachineIRBuilder &MIRBuilder,
  ...
  if (!VRegs.empty()) {
    ...
    const FgpuTargetLowering &TLI = *getTLI<FgpuTargetLowering>();
    ...
    FgpuCCState CCInfo(F.getCallingConv(), F.isVarArg(), MF, ArgLocs,
                       F.getContext());

    FgpuOutgoingValueHandler RetHandler(MIRBuilder, MF.getRegInfo(), Ret);
    std::string FuncName = F.getName().str();
    FgpuOutgoingValueAssigner Assigner(TLI.CCAssignFnForReturn(),
...
bool FgpuCallLowering::lowerFormalArguments(MachineIRBuilder &MIRBuilder,
                                            const Function &F,
  ...
  const FgpuTargetMachine &TM =
      static_cast<const FgpuTargetMachine &>(MF.getTarget());
  const FgpuABIInfo &ABI = TM.getABI();
  CCInfo.AllocateStack(ABI.GetCalleeAllocdArgSizeInBytes(F.getCallingConv()),
                       Align(1));

  const std::string FuncName = F.getName().str();
  FgpuIncomingValueAssigner Assigner(TLI.CCAssignFnForCall(), FuncName.c_str(),
                                     /*IsReturn*/ false);
  if (!determineAssignments(Assigner, ArgInfos, CCInfo))
    return false;

  FgpuIncomingValueHandler Handler(MIRBuilder, MF.getRegInfo());
  ...
  if (F.isVarArg()) {
    ...
    MF.getInfo<FgpuFunctionInfo>()->setVarArgsFrameIndex(FI);
...
bool FgpuCallLowering::lowerCall(MachineIRBuilder &MIRBuilder,
  ...
  MachineInstrBuilder CallSeqStart =
      MIRBuilder.buildInstr(Fgpu::ADJCALLSTACKDOWN);
  ...
  if (IsCalleeGlobalPIC) {
    ...
    if (!Info.Callee.getGlobal()->hasLocalLinkage())
      CalleeGlobalValue->getOperand(1).setTargetFlags(FgpuII::MO_GOT_CALL);
  ...
  // FIXME: Should use FgpuCCState::getSpecialCallingConvForCallee, but it
  // depends on looking directly at the call target.
  FgpuCCState CCInfo(Info.CallConv, IsCalleeVarArg, MF, ArgLocs,
                     F.getContext());
  ...
  FgpuOutgoingValueAssigner Assigner(TLI.CCAssignFnForCall(), Call,
                                     /*IsReturn*/ false);
  if (!determineAssignments(Assigner, ArgInfos, CCInfo))
    return false;

  FgpuOutgoingValueHandler ArgHandler(MIRBuilder, MF.getRegInfo(), MIB);
  ...
  if (!Info.OrigRet.Ty->isVoidTy()) {
    ...
    SmallVector<ISD::InputArg, 8> Ins;
    SmallVector<CCValAssign, 8> ArgLocs;
    FgpuIncomingValueAssigner Assigner(TLI.CCAssignFnForReturn(),
                                       FuncName.c_str(),
                                       /*IsReturn*/ true);
  ...
  MIRBuilder.buildInstr(Fgpu::ADJCALLSTACKUP).addImm(NextStackOffset).addImm(0);
