//===-- FgpuMCExpr.cpp - Fgpu specific MC expression classes --------------===//
//
...
const FgpuMCExpr *FgpuMCExpr::create(FgpuMCExpr::FgpuExprKind Kind,
  ...
  return new (Ctx) FgpuMCExpr(Kind, Expr);
}

const FgpuMCExpr *FgpuMCExpr::createGpOff(FgpuMCExpr::FgpuExprKind Kind,
                                          const MCExpr *Expr, MCContext &Ctx) {
  return create(Kind, create(MEK_NEG, create(MEK_GPREL, Expr, Ctx), Ctx), Ctx);
}

void FgpuMCExpr::printImpl(raw_ostream &OS, const MCAsmInfo *MAI) const {
  int64_t AbsVal;
...
bool
FgpuMCExpr::evaluateAsRelocatableImpl(MCValue &Res,
                                      const MCAsmLayout *Layout,
                                      const MCFixup *Fixup) const {
  // Look for the %hi(%neg(%gp_rel(X))) and %lo(%neg(%gp_rel(X))) special cases.
  if (isGpOff()) {
    const MCExpr *SubExpr =
        cast<FgpuMCExpr>(cast<FgpuMCExpr>(getSubExpr())->getSubExpr())
            ->getSubExpr();
...
void FgpuMCExpr::visitUsedExpr(MCStreamer &Streamer) const {
...
static void fixELFSymbolsInTLSFixupsImpl(const MCExpr *Expr, MCAssembler &Asm) {
  switch (Expr->getKind()) {
  case MCExpr::Target:
    fixELFSymbolsInTLSFixupsImpl(cast<FgpuMCExpr>(Expr)->getSubExpr(), Asm);
    break;
...
void FgpuMCExpr::fixELFSymbolsInTLSFixups(MCAssembler &Asm) const {
  switch (getKind()) {
  case MEK_None:
  case MEK_Special:
    llvm_unreachable("MEK_None and MEK_Special are invalid");
  ...
  case MEK_PCREL_LO16:
    ...
    // a consecutive chain.
    if (const FgpuMCExpr *E = dyn_cast<const FgpuMCExpr>(getSubExpr()))
...
bool FgpuMCExpr::isGpOff(FgpuExprKind &Kind) const {
  if (getKind() == MEK_HI || getKind() == MEK_LO) {
    if (const FgpuMCExpr *S1 = dyn_cast<const FgpuMCExpr>(getSubExpr())) {
      if (const FgpuMCExpr *S2 = dyn_cast<const FgpuMCExpr>(S1->getSubExpr())) {
