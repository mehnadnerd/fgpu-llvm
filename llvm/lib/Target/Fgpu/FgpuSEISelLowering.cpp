...
FgpuSETargetLowering::FgpuSETargetLowering(const FgpuTargetMachine &TM,
  ...
  addRegisterClass(MVT::i32, &Fgpu::GPR32RegClass);

  if (Subtarget.isGP64bit())
    addRegisterClass(MVT::i64, &Fgpu::GPR64RegClass);
  ...
  if (Subtarget.hasDSP()) {
    ...
    for (unsigned i = 0; i < array_lengthof(VecTys); ++i) {
      addRegisterClass(VecTys[i], &Fgpu::DSPRRegClass);
  ...
  if (Subtarget.hasMSA()) {
    addMSAIntType(MVT::v16i8, &Fgpu::MSA128BRegClass);
    addMSAIntType(MVT::v8i16, &Fgpu::MSA128HRegClass);
    addMSAIntType(MVT::v4i32, &Fgpu::MSA128WRegClass);
    addMSAIntType(MVT::v2i64, &Fgpu::MSA128DRegClass);
    addMSAFloatType(MVT::v8f16, &Fgpu::MSA128HRegClass);
    addMSAFloatType(MVT::v4f32, &Fgpu::MSA128WRegClass);
    addMSAFloatType(MVT::v2f64, &Fgpu::MSA128DRegClass);

    // f16 is a storage-only type, always promote it to f32.
    addRegisterClass(MVT::f16, &Fgpu::MSA128HRegClass);
  ...
  if (Subtarget.hasFgpu32r2() && !Subtarget.useSoftFloat() &&
      !Subtarget.hasFgpu64()) {
  ...
  if (Subtarget.hasFgpu32r6()) {
    // FGPU32r6 replaces the accumulator-based multiplies with a three register
    // instruction
    ...
    // FGPU32r6 replaces the accumulator-based division/remainder with separate
    ...
    // FGPU32r6 replaces conditional moves with an equivalent that removes the
    ...
    assert(Subtarget.isFP64bit() && "FR=1 is required for FGPU32r6");
  ...
  if (Subtarget.hasFgpu64r6()) {
    // FGPU64r6 replaces the accumulator-based multiplies with a three register
    // instruction
    ...
    // FGPU64r6 replaces conditional moves with an equivalent that removes the
    // need for three GPR read ports.
...
FgpuSETargetLowering::getRepRegClassFor(MVT VT) const {
  if (VT == MVT::Untyped)
    return Subtarget.hasDSP() ? &Fgpu::ACC64DSPRegClass : &Fgpu::ACC64RegClass;
...
SDValue FgpuSETargetLowering::LowerOperation(SDValue Op,
  ...
  case ISD::SMUL_LOHI: return lowerMulDiv(Op, FgpuISD::Mult, true, true, DAG);
  case ISD::UMUL_LOHI: return lowerMulDiv(Op, FgpuISD::Multu, true, true, DAG);
  case ISD::MULHS:     return lowerMulDiv(Op, FgpuISD::Mult, false, true, DAG);
  case ISD::MULHU:     return lowerMulDiv(Op, FgpuISD::Multu, false, true, DAG);
  case ISD::MUL:       return lowerMulDiv(Op, FgpuISD::Mult, true, false, DAG);
  case ISD::SDIVREM:   return lowerMulDiv(Op, FgpuISD::DivRem, true, true, DAG);
  case ISD::UDIVREM:   return lowerMulDiv(Op, FgpuISD::DivRemU, true, true,
                                          DAG);
...
// Fold zero extensions into FgpuISD::VEXTRACT_[SZ]EXT_ELT
//
// Performs the following transformations:
// - Changes FgpuISD::VEXTRACT_[SZ]EXT_ELT to zero extension if its
...
static SDValue performANDCombine(SDNode *N, SelectionDAG &DAG,
  ...
  // (and (FgpuVExtract[SZ]Ext $a, $b, $c), imm:$d)
  // where $d + 1 == 2^n and n == 32
  // or    $d + 1 == 2^n and n <= 32 and ZExt
  // -> (FgpuVExtractZExt $a, $b, $c)
  if (Op0Opcode == FgpuISD::VEXTRACT_SEXT_ELT ||
      Op0Opcode == FgpuISD::VEXTRACT_ZEXT_ELT) {
    ...
    if ((Op0Opcode == FgpuISD::VEXTRACT_ZEXT_ELT && Log2 >= ExtendTySize) ||
        Log2 == ExtendTySize) {
      SDValue Ops[] = { Op0->getOperand(0), Op0->getOperand(1), Op0Op2 };
      return DAG.getNode(FgpuISD::VEXTRACT_ZEXT_ELT, SDLoc(Op0),
                         Op0->getVTList(),
...
static SDValue performSHLCombine(SDNode *N, SelectionDAG &DAG,
  ...
  return performDSPShiftCombine(FgpuISD::SHLL_DSP, N, Ty, DAG, Subtarget);
}

// Fold sign-extensions into FgpuISD::VEXTRACT_[SZ]EXT_ELT for MSA and fold
// constant splats into FgpuISD::SHRA_DSP for DSPr2.
//
// Performs the following transformations:
// - Changes FgpuISD::VEXTRACT_[SZ]EXT_ELT to sign extension if its
//   sign/zero-extension is completely overwritten by the new one performed by
...
static SDValue performSRACombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (Subtarget.hasMSA()) {
    ...
    // (sra (shl (FgpuVExtract[SZ]Ext $a, $b, $c), imm:$d), imm:$d)
    // where $d + sizeof($c) == 32
    // or    $d + sizeof($c) <= 32 and SExt
    // -> (FgpuVExtractSExt $a, $b, $c)
    if (Op0->getOpcode() == ISD::SHL && Op1 == Op0->getOperand(1)) {
      ...
      if (Op0Op0->getOpcode() != FgpuISD::VEXTRACT_SEXT_ELT &&
          Op0Op0->getOpcode() != FgpuISD::VEXTRACT_ZEXT_ELT)
        return SDValue();
      ...
      if (TotalBits == 32 ||
          (Op0Op0->getOpcode() == FgpuISD::VEXTRACT_SEXT_ELT &&
           TotalBits <= 32)) {
        SDValue Ops[] = { Op0Op0->getOperand(0), Op0Op0->getOperand(1),
                          Op0Op0->getOperand(2) };
        return DAG.getNode(FgpuISD::VEXTRACT_SEXT_ELT, SDLoc(Op0Op0),
                           Op0Op0->getVTList(),
  ...
  return performDSPShiftCombine(FgpuISD::SHRA_DSP, N, Ty, DAG, Subtarget);
}
...
static SDValue performSRLCombine(SDNode *N, SelectionDAG &DAG,
  ...
  return performDSPShiftCombine(FgpuISD::SHRL_DSP, N, Ty, DAG, Subtarget);
}
...
static SDValue performSETCCCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  return DAG.getNode(FgpuISD::SETCC_DSP, SDLoc(N), Ty, N->getOperand(0),
...
static SDValue performVSELECTCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  if (Ty == MVT::v2i16 || Ty == MVT::v4i8) {
    ...
    if (SetCC.getOpcode() != FgpuISD::SETCC_DSP)
      return SDValue();

    return DAG.getNode(FgpuISD::SELECT_CC_DSP, SDLoc(N), Ty,
...
static SDValue performXORCombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (Subtarget.hasMSA() && Ty.is128BitVector() && Ty.isInteger()) {
    ...
    if (NotOp->getOpcode() == ISD::OR)
      return DAG.getNode(FgpuISD::VNOR, SDLoc(N), Ty, NotOp->getOperand(0),
                         NotOp->getOperand(1));
  }
...
FgpuSETargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  case Fgpu::BPOSGE32_PSEUDO:
    return emitBPOSGE32(MI, BB);
  case Fgpu::SNZ_B_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Fgpu::BNZ_B);
  case Fgpu::SNZ_H_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Fgpu::BNZ_H);
  case Fgpu::SNZ_W_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Fgpu::BNZ_W);
  case Fgpu::SNZ_D_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Fgpu::BNZ_D);
  case Fgpu::SNZ_V_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Fgpu::BNZ_V);
  case Fgpu::SZ_B_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Fgpu::BZ_B);
  case Fgpu::SZ_H_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Fgpu::BZ_H);
  case Fgpu::SZ_W_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Fgpu::BZ_W);
  case Fgpu::SZ_D_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Fgpu::BZ_D);
  case Fgpu::SZ_V_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, Fgpu::BZ_V);
  case Fgpu::COPY_FW_PSEUDO:
    return emitCOPY_FW(MI, BB);
  case Fgpu::COPY_FD_PSEUDO:
    return emitCOPY_FD(MI, BB);
  case Fgpu::INSERT_FW_PSEUDO:
    return emitINSERT_FW(MI, BB);
  case Fgpu::INSERT_FD_PSEUDO:
    return emitINSERT_FD(MI, BB);
  case Fgpu::INSERT_B_VIDX_PSEUDO:
  case Fgpu::INSERT_B_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 1, false);
  case Fgpu::INSERT_H_VIDX_PSEUDO:
  case Fgpu::INSERT_H_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 2, false);
  case Fgpu::INSERT_W_VIDX_PSEUDO:
  case Fgpu::INSERT_W_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 4, false);
  case Fgpu::INSERT_D_VIDX_PSEUDO:
  case Fgpu::INSERT_D_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 8, false);
  case Fgpu::INSERT_FW_VIDX_PSEUDO:
  case Fgpu::INSERT_FW_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 4, true);
  case Fgpu::INSERT_FD_VIDX_PSEUDO:
  case Fgpu::INSERT_FD_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 8, true);
  case Fgpu::FILL_FW_PSEUDO:
    return emitFILL_FW(MI, BB);
  case Fgpu::FILL_FD_PSEUDO:
    return emitFILL_FD(MI, BB);
  case Fgpu::FEXP2_W_1_PSEUDO:
    return emitFEXP2_W_1(MI, BB);
  case Fgpu::FEXP2_D_1_PSEUDO:
    return emitFEXP2_D_1(MI, BB);
  case Fgpu::ST_F16:
    return emitST_F16_PSEUDO(MI, BB);
  case Fgpu::LD_F16:
    return emitLD_F16_PSEUDO(MI, BB);
  case Fgpu::MSA_FP_EXTEND_W_PSEUDO:
    return emitFPEXTEND_PSEUDO(MI, BB, false);
  case Fgpu::MSA_FP_ROUND_W_PSEUDO:
    return emitFPROUND_PSEUDO(MI, BB, false);
  case Fgpu::MSA_FP_EXTEND_D_PSEUDO:
...
SDValue FgpuSETargetLowering::lowerLOAD(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue BP = DAG.getNode(FgpuISD::BuildPairF64, DL, MVT::f64, Lo, Hi);
...
SDValue FgpuSETargetLowering::lowerBITCAST(SDValue Op,
  ...
  if (Src == MVT::i64 && Dest == MVT::f64) {
    ...
    return DAG.getNode(FgpuISD::BuildPairF64, DL, MVT::f64, Lo, Hi);
  }
...
SDValue FgpuSETargetLowering::lowerMulDiv(SDValue Op, unsigned NewOpc,
                                          bool HasLo, bool HasHi,
                                          SelectionDAG &DAG) const {
  // FGPU32r6/FGPU64r6 removed accumulator based multiplies.
  assert(!Subtarget.hasFgpu32r6());
  ...
  if (HasLo)
    Lo = DAG.getNode(FgpuISD::MFLO, DL, Ty, Mult);
  if (HasHi)
    Hi = DAG.getNode(FgpuISD::MFHI, DL, Ty, Mult);
...
static SDValue initAccumulator(SDValue In, const SDLoc &DL, SelectionDAG &DAG) {
  ...
  SDValue InHi = DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i32, In,
                             DAG.getConstant(1, DL, MVT::i32));
  return DAG.getNode(FgpuISD::MTLOHI, DL, MVT::Untyped, InLo, InHi);
}

static SDValue extractLOHI(SDValue Op, const SDLoc &DL, SelectionDAG &DAG) {
  SDValue Lo = DAG.getNode(FgpuISD::MFLO, DL, MVT::i32, Op);
  SDValue Hi = DAG.getNode(FgpuISD::MFHI, DL, MVT::i32, Op);
  return DAG.getNode(ISD::BUILD_PAIR, DL, MVT::i64, Lo, Hi);
}

// This function expands fgpu intrinsic nodes which have 64-bit input operands
// or output values.
//
...
SDValue FgpuSETargetLowering::lowerINTRINSIC_WO_CHAIN(SDValue Op,
  ...
  default:
    return SDValue();
  case Intrinsic::fgpu_shilo:
    return lowerDSPIntr(Op, DAG, FgpuISD::SHILO);
  case Intrinsic::fgpu_dpau_h_qbl:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPAU_H_QBL);
  case Intrinsic::fgpu_dpau_h_qbr:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPAU_H_QBR);
  case Intrinsic::fgpu_dpsu_h_qbl:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPSU_H_QBL);
  case Intrinsic::fgpu_dpsu_h_qbr:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPSU_H_QBR);
  case Intrinsic::fgpu_dpa_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPA_W_PH);
  case Intrinsic::fgpu_dps_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPS_W_PH);
  case Intrinsic::fgpu_dpax_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPAX_W_PH);
  case Intrinsic::fgpu_dpsx_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPSX_W_PH);
  case Intrinsic::fgpu_mulsa_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::MULSA_W_PH);
  case Intrinsic::fgpu_mult:
    return lowerDSPIntr(Op, DAG, FgpuISD::Mult);
  case Intrinsic::fgpu_multu:
    return lowerDSPIntr(Op, DAG, FgpuISD::Multu);
  case Intrinsic::fgpu_madd:
    return lowerDSPIntr(Op, DAG, FgpuISD::MAdd);
  case Intrinsic::fgpu_maddu:
    return lowerDSPIntr(Op, DAG, FgpuISD::MAddu);
  case Intrinsic::fgpu_msub:
    return lowerDSPIntr(Op, DAG, FgpuISD::MSub);
  case Intrinsic::fgpu_msubu:
    return lowerDSPIntr(Op, DAG, FgpuISD::MSubu);
  ...
  case Intrinsic::fgpu_and_v:
    return DAG.getNode(ISD::AND, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::fgpu_andi_b:
    return DAG.getNode(ISD::AND, DL, Op->getValueType(0), Op->getOperand(1),
                       lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::fgpu_bclr_b:
  case Intrinsic::fgpu_bclr_h:
  case Intrinsic::fgpu_bclr_w:
  case Intrinsic::fgpu_bclr_d:
    return lowerMSABitClear(Op, DAG);
  case Intrinsic::fgpu_bclri_b:
  case Intrinsic::fgpu_bclri_h:
  case Intrinsic::fgpu_bclri_w:
  case Intrinsic::fgpu_bclri_d:
    return lowerMSABitClearImm(Op, DAG);
  case Intrinsic::fgpu_binsli_b:
  case Intrinsic::fgpu_binsli_h:
  case Intrinsic::fgpu_binsli_w:
  case Intrinsic::fgpu_binsli_d: {
  ...
  case Intrinsic::fgpu_binsri_b:
  case Intrinsic::fgpu_binsri_h:
  case Intrinsic::fgpu_binsri_w:
  case Intrinsic::fgpu_binsri_d: {
  ...
  case Intrinsic::fgpu_bmnz_v:
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0), Op->getOperand(3),
                       Op->getOperand(2), Op->getOperand(1));
  case Intrinsic::fgpu_bmnzi_b:
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0),
                       lowerMSASplatImm(Op, 3, DAG), Op->getOperand(2),
                       Op->getOperand(1));
  case Intrinsic::fgpu_bmz_v:
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0), Op->getOperand(3),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_bmzi_b:
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0),
                       lowerMSASplatImm(Op, 3, DAG), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::fgpu_bneg_b:
  case Intrinsic::fgpu_bneg_h:
  case Intrinsic::fgpu_bneg_w:
  case Intrinsic::fgpu_bneg_d: {
    EVT VecTy = Op->getValueType(0);
  ...
  case Intrinsic::fgpu_bnegi_b:
  case Intrinsic::fgpu_bnegi_h:
  case Intrinsic::fgpu_bnegi_w:
  case Intrinsic::fgpu_bnegi_d:
    return lowerMSABinaryBitImmIntr(Op, DAG, ISD::XOR, Op->getOperand(2),
                                    !Subtarget.isLittle());
  case Intrinsic::fgpu_bnz_b:
  case Intrinsic::fgpu_bnz_h:
  case Intrinsic::fgpu_bnz_w:
  case Intrinsic::fgpu_bnz_d:
    return DAG.getNode(FgpuISD::VALL_NONZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::fgpu_bnz_v:
    return DAG.getNode(FgpuISD::VANY_NONZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::fgpu_bsel_v:
    // bsel_v(Mask, IfClear, IfSet) -> (vselect Mask, IfSet, IfClear)
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(3),
                       Op->getOperand(2));
  case Intrinsic::fgpu_bseli_b:
    // bseli_v(Mask, IfClear, IfSet) -> (vselect Mask, IfSet, IfClear)
    return DAG.getNode(ISD::VSELECT, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 3, DAG),
                       Op->getOperand(2));
  case Intrinsic::fgpu_bset_b:
  case Intrinsic::fgpu_bset_h:
  case Intrinsic::fgpu_bset_w:
  case Intrinsic::fgpu_bset_d: {
    EVT VecTy = Op->getValueType(0);
    SDValue One = DAG.getConstant(1, DL, VecTy);
  ...
  case Intrinsic::fgpu_bseti_b:
  case Intrinsic::fgpu_bseti_h:
  case Intrinsic::fgpu_bseti_w:
  case Intrinsic::fgpu_bseti_d:
    return lowerMSABinaryBitImmIntr(Op, DAG, ISD::OR, Op->getOperand(2),
                                    !Subtarget.isLittle());
  case Intrinsic::fgpu_bz_b:
  case Intrinsic::fgpu_bz_h:
  case Intrinsic::fgpu_bz_w:
  case Intrinsic::fgpu_bz_d:
    return DAG.getNode(FgpuISD::VALL_ZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::fgpu_bz_v:
    return DAG.getNode(FgpuISD::VANY_ZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::fgpu_ceq_b:
  case Intrinsic::fgpu_ceq_h:
  case Intrinsic::fgpu_ceq_w:
  case Intrinsic::fgpu_ceq_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETEQ);
  case Intrinsic::fgpu_ceqi_b:
  case Intrinsic::fgpu_ceqi_h:
  case Intrinsic::fgpu_ceqi_w:
  case Intrinsic::fgpu_ceqi_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        lowerMSASplatImm(Op, 2, DAG, true), ISD::SETEQ);
  case Intrinsic::fgpu_cle_s_b:
  case Intrinsic::fgpu_cle_s_h:
  case Intrinsic::fgpu_cle_s_w:
  case Intrinsic::fgpu_cle_s_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETLE);
  case Intrinsic::fgpu_clei_s_b:
  case Intrinsic::fgpu_clei_s_h:
  case Intrinsic::fgpu_clei_s_w:
  case Intrinsic::fgpu_clei_s_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        lowerMSASplatImm(Op, 2, DAG, true), ISD::SETLE);
  case Intrinsic::fgpu_cle_u_b:
  case Intrinsic::fgpu_cle_u_h:
  case Intrinsic::fgpu_cle_u_w:
  case Intrinsic::fgpu_cle_u_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETULE);
  case Intrinsic::fgpu_clei_u_b:
  case Intrinsic::fgpu_clei_u_h:
  case Intrinsic::fgpu_clei_u_w:
  case Intrinsic::fgpu_clei_u_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        lowerMSASplatImm(Op, 2, DAG), ISD::SETULE);
  case Intrinsic::fgpu_clt_s_b:
  case Intrinsic::fgpu_clt_s_h:
  case Intrinsic::fgpu_clt_s_w:
  case Intrinsic::fgpu_clt_s_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETLT);
  case Intrinsic::fgpu_clti_s_b:
  case Intrinsic::fgpu_clti_s_h:
  case Intrinsic::fgpu_clti_s_w:
  case Intrinsic::fgpu_clti_s_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        lowerMSASplatImm(Op, 2, DAG, true), ISD::SETLT);
  case Intrinsic::fgpu_clt_u_b:
  case Intrinsic::fgpu_clt_u_h:
  case Intrinsic::fgpu_clt_u_w:
  case Intrinsic::fgpu_clt_u_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETULT);
  case Intrinsic::fgpu_clti_u_b:
  case Intrinsic::fgpu_clti_u_h:
  case Intrinsic::fgpu_clti_u_w:
  case Intrinsic::fgpu_clti_u_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        lowerMSASplatImm(Op, 2, DAG), ISD::SETULT);
  case Intrinsic::fgpu_copy_s_b:
  case Intrinsic::fgpu_copy_s_h:
  case Intrinsic::fgpu_copy_s_w:
    return lowerMSACopyIntr(Op, DAG, FgpuISD::VEXTRACT_SEXT_ELT);
  case Intrinsic::fgpu_copy_s_d:
    if (Subtarget.hasFgpu64())
      // Lower directly into VEXTRACT_SEXT_ELT since i64 is legal on Fgpu64.
      return lowerMSACopyIntr(Op, DAG, FgpuISD::VEXTRACT_SEXT_ELT);
    else {
  ...
  case Intrinsic::fgpu_copy_u_b:
  case Intrinsic::fgpu_copy_u_h:
  case Intrinsic::fgpu_copy_u_w:
    return lowerMSACopyIntr(Op, DAG, FgpuISD::VEXTRACT_ZEXT_ELT);
  case Intrinsic::fgpu_copy_u_d:
    if (Subtarget.hasFgpu64())
      // Lower directly into VEXTRACT_ZEXT_ELT since i64 is legal on Fgpu64.
      return lowerMSACopyIntr(Op, DAG, FgpuISD::VEXTRACT_ZEXT_ELT);
    else {
      // Lower into the generic EXTRACT_VECTOR_ELT node and let the type
      // legalizer and EXTRACT_VECTOR_ELT lowering sort it out.
  ...
  case Intrinsic::fgpu_fadd_d:
    ...
    return DAG.getNode(ISD::FADD, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  // Don't lower fgpu_fcaf_[wd] since LLVM folds SETFALSE condcodes away
  case Intrinsic::fgpu_fceq_w:
  case Intrinsic::fgpu_fceq_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETOEQ);
  case Intrinsic::fgpu_fcle_w:
  case Intrinsic::fgpu_fcle_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETOLE);
  case Intrinsic::fgpu_fclt_w:
  case Intrinsic::fgpu_fclt_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETOLT);
  case Intrinsic::fgpu_fcne_w:
  case Intrinsic::fgpu_fcne_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETONE);
  case Intrinsic::fgpu_fcor_w:
  case Intrinsic::fgpu_fcor_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETO);
  case Intrinsic::fgpu_fcueq_w:
  case Intrinsic::fgpu_fcueq_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETUEQ);
  case Intrinsic::fgpu_fcule_w:
  case Intrinsic::fgpu_fcule_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETULE);
  case Intrinsic::fgpu_fcult_w:
  case Intrinsic::fgpu_fcult_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETULT);
  case Intrinsic::fgpu_fcun_w:
  case Intrinsic::fgpu_fcun_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
                        Op->getOperand(2), ISD::SETUO);
  case Intrinsic::fgpu_fcune_w:
  case Intrinsic::fgpu_fcune_d:
    return DAG.getSetCC(DL, Op->getValueType(0), Op->getOperand(1),
  ...
  case Intrinsic::fgpu_fdiv_d:
    ...
    return DAG.getNode(ISD::FDIV, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::fgpu_ffint_u_w:
  case Intrinsic::fgpu_ffint_u_d:
    return DAG.getNode(ISD::UINT_TO_FP, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::fgpu_ffint_s_w:
  case Intrinsic::fgpu_ffint_s_d:
    return DAG.getNode(ISD::SINT_TO_FP, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::fgpu_fill_b:
  case Intrinsic::fgpu_fill_h:
  case Intrinsic::fgpu_fill_w:
  case Intrinsic::fgpu_fill_d: {
    EVT ResTy = Op->getValueType(0);
  ...
  case Intrinsic::fgpu_fexp2_w:
  case Intrinsic::fgpu_fexp2_d: {
    // TODO: If intrinsics have fast-math-flags, propagate them.
    EVT ResTy = Op->getValueType(0);
    return DAG.getNode(
  ...
  case Intrinsic::fgpu_flog2_w:
  case Intrinsic::fgpu_flog2_d:
    return DAG.getNode(ISD::FLOG2, DL, Op->getValueType(0), Op->getOperand(1));
  case Intrinsic::fgpu_fmadd_w:
  case Intrinsic::fgpu_fmadd_d:
  ...
  case Intrinsic::fgpu_fmul_d:
    ...
    return DAG.getNode(ISD::FMUL, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::fgpu_fmsub_w:
  case Intrinsic::fgpu_fmsub_d: {
    // TODO: If intrinsics have fast-math-flags, propagate them.
    return DAG.getNode(FgpuISD::FMS, SDLoc(Op), Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2), Op->getOperand(3));
  }
  case Intrinsic::fgpu_frint_w:
  case Intrinsic::fgpu_frint_d:
  ...
  case Intrinsic::fgpu_fsub_d:
    ...
    return DAG.getNode(ISD::FSUB, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::fgpu_ftrunc_u_w:
  case Intrinsic::fgpu_ftrunc_u_d:
    return DAG.getNode(ISD::FP_TO_UINT, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::fgpu_ftrunc_s_w:
  case Intrinsic::fgpu_ftrunc_s_d:
    return DAG.getNode(ISD::FP_TO_SINT, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::fgpu_ilvev_b:
  case Intrinsic::fgpu_ilvev_h:
  case Intrinsic::fgpu_ilvev_w:
  case Intrinsic::fgpu_ilvev_d:
    return DAG.getNode(FgpuISD::ILVEV, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_ilvl_b:
  case Intrinsic::fgpu_ilvl_h:
  case Intrinsic::fgpu_ilvl_w:
  case Intrinsic::fgpu_ilvl_d:
    return DAG.getNode(FgpuISD::ILVL, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_ilvod_b:
  case Intrinsic::fgpu_ilvod_h:
  case Intrinsic::fgpu_ilvod_w:
  case Intrinsic::fgpu_ilvod_d:
    return DAG.getNode(FgpuISD::ILVOD, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_ilvr_b:
  case Intrinsic::fgpu_ilvr_h:
  case Intrinsic::fgpu_ilvr_w:
  case Intrinsic::fgpu_ilvr_d:
    return DAG.getNode(FgpuISD::ILVR, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_insert_b:
  case Intrinsic::fgpu_insert_h:
  case Intrinsic::fgpu_insert_w:
  case Intrinsic::fgpu_insert_d:
    return DAG.getNode(ISD::INSERT_VECTOR_ELT, SDLoc(Op), Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(3), Op->getOperand(2));
  case Intrinsic::fgpu_insve_b:
  case Intrinsic::fgpu_insve_h:
  case Intrinsic::fgpu_insve_w:
  case Intrinsic::fgpu_insve_d: {
    // Report an error for out of range values.
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::fgpu_insve_b: Max = 15; break;
    case Intrinsic::fgpu_insve_h: Max = 7; break;
    case Intrinsic::fgpu_insve_w: Max = 3; break;
    case Intrinsic::fgpu_insve_d: Max = 1; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > Max)
      report_fatal_error("Immediate out of range");
    return DAG.getNode(FgpuISD::INSVE, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2), Op->getOperand(3),
                       DAG.getConstant(0, DL, MVT::i32));
    }
  case Intrinsic::fgpu_ldi_b:
  case Intrinsic::fgpu_ldi_h:
  case Intrinsic::fgpu_ldi_w:
  case Intrinsic::fgpu_ldi_d:
    return lowerMSASplatImm(Op, 1, DAG, true);
  case Intrinsic::fgpu_lsa:
  case Intrinsic::fgpu_dlsa: {
    EVT ResTy = Op->getValueType(0);
    return DAG.getNode(ISD::ADD, SDLoc(Op), ResTy, Op->getOperand(1),
                       DAG.getNode(ISD::SHL, SDLoc(Op), ResTy,
                                   Op->getOperand(2), Op->getOperand(3)));
  }
  case Intrinsic::fgpu_maddv_b:
  case Intrinsic::fgpu_maddv_h:
  case Intrinsic::fgpu_maddv_w:
  case Intrinsic::fgpu_maddv_d: {
    EVT ResTy = Op->getValueType(0);
    return DAG.getNode(ISD::ADD, SDLoc(Op), ResTy, Op->getOperand(1),
                       DAG.getNode(ISD::MUL, SDLoc(Op), ResTy,
                                   Op->getOperand(2), Op->getOperand(3)));
  }
  case Intrinsic::fgpu_max_s_b:
  case Intrinsic::fgpu_max_s_h:
  case Intrinsic::fgpu_max_s_w:
  case Intrinsic::fgpu_max_s_d:
    return DAG.getNode(ISD::SMAX, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_max_u_b:
  case Intrinsic::fgpu_max_u_h:
  case Intrinsic::fgpu_max_u_w:
  case Intrinsic::fgpu_max_u_d:
    return DAG.getNode(ISD::UMAX, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_maxi_s_b:
  case Intrinsic::fgpu_maxi_s_h:
  case Intrinsic::fgpu_maxi_s_w:
  case Intrinsic::fgpu_maxi_s_d:
    return DAG.getNode(ISD::SMAX, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG, true));
  case Intrinsic::fgpu_maxi_u_b:
  case Intrinsic::fgpu_maxi_u_h:
  case Intrinsic::fgpu_maxi_u_w:
  case Intrinsic::fgpu_maxi_u_d:
    return DAG.getNode(ISD::UMAX, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::fgpu_min_s_b:
  case Intrinsic::fgpu_min_s_h:
  case Intrinsic::fgpu_min_s_w:
  case Intrinsic::fgpu_min_s_d:
    return DAG.getNode(ISD::SMIN, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_min_u_b:
  case Intrinsic::fgpu_min_u_h:
  case Intrinsic::fgpu_min_u_w:
  case Intrinsic::fgpu_min_u_d:
    return DAG.getNode(ISD::UMIN, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_mini_s_b:
  case Intrinsic::fgpu_mini_s_h:
  case Intrinsic::fgpu_mini_s_w:
  case Intrinsic::fgpu_mini_s_d:
    return DAG.getNode(ISD::SMIN, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG, true));
  case Intrinsic::fgpu_mini_u_b:
  case Intrinsic::fgpu_mini_u_h:
  case Intrinsic::fgpu_mini_u_w:
  case Intrinsic::fgpu_mini_u_d:
    return DAG.getNode(ISD::UMIN, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  ...
  case Intrinsic::fgpu_mulv_d:
    return DAG.getNode(ISD::MUL, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::fgpu_msubv_b:
  case Intrinsic::fgpu_msubv_h:
  case Intrinsic::fgpu_msubv_w:
  case Intrinsic::fgpu_msubv_d: {
    EVT ResTy = Op->getValueType(0);
    return DAG.getNode(ISD::SUB, SDLoc(Op), ResTy, Op->getOperand(1),
                       DAG.getNode(ISD::MUL, SDLoc(Op), ResTy,
                                   Op->getOperand(2), Op->getOperand(3)));
  }
  case Intrinsic::fgpu_nlzc_b:
  case Intrinsic::fgpu_nlzc_h:
  case Intrinsic::fgpu_nlzc_w:
  case Intrinsic::fgpu_nlzc_d:
    return DAG.getNode(ISD::CTLZ, DL, Op->getValueType(0), Op->getOperand(1));
  case Intrinsic::fgpu_nor_v: {
    SDValue Res = DAG.getNode(ISD::OR, DL, Op->getValueType(0),
                              Op->getOperand(1), Op->getOperand(2));
    return DAG.getNOT(DL, Res, Res->getValueType(0));
  }
  case Intrinsic::fgpu_nori_b: {
    SDValue Res =  DAG.getNode(ISD::OR, DL, Op->getValueType(0),
                               Op->getOperand(1),
                               lowerMSASplatImm(Op, 2, DAG));
    return DAG.getNOT(DL, Res, Res->getValueType(0));
  }
  case Intrinsic::fgpu_or_v:
    return DAG.getNode(ISD::OR, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::fgpu_ori_b:
    return DAG.getNode(ISD::OR, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::fgpu_pckev_b:
  case Intrinsic::fgpu_pckev_h:
  case Intrinsic::fgpu_pckev_w:
  case Intrinsic::fgpu_pckev_d:
    return DAG.getNode(FgpuISD::PCKEV, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_pckod_b:
  case Intrinsic::fgpu_pckod_h:
  case Intrinsic::fgpu_pckod_w:
  case Intrinsic::fgpu_pckod_d:
    return DAG.getNode(FgpuISD::PCKOD, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::fgpu_pcnt_b:
  case Intrinsic::fgpu_pcnt_h:
  case Intrinsic::fgpu_pcnt_w:
  case Intrinsic::fgpu_pcnt_d:
    return DAG.getNode(ISD::CTPOP, DL, Op->getValueType(0), Op->getOperand(1));
  case Intrinsic::fgpu_sat_s_b:
  case Intrinsic::fgpu_sat_s_h:
  case Intrinsic::fgpu_sat_s_w:
  case Intrinsic::fgpu_sat_s_d:
  case Intrinsic::fgpu_sat_u_b:
  case Intrinsic::fgpu_sat_u_h:
  case Intrinsic::fgpu_sat_u_w:
  case Intrinsic::fgpu_sat_u_d: {
    // Report an error for out of range values.
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::fgpu_sat_s_b:
    case Intrinsic::fgpu_sat_u_b: Max = 7;  break;
    case Intrinsic::fgpu_sat_s_h:
    case Intrinsic::fgpu_sat_u_h: Max = 15; break;
    case Intrinsic::fgpu_sat_s_w:
    case Intrinsic::fgpu_sat_u_w: Max = 31; break;
    case Intrinsic::fgpu_sat_s_d:
    case Intrinsic::fgpu_sat_u_d: Max = 63; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > Max)
  ...
  case Intrinsic::fgpu_shf_b:
  case Intrinsic::fgpu_shf_h:
  case Intrinsic::fgpu_shf_w: {
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > 255)
      report_fatal_error("Immediate out of range");
    return DAG.getNode(FgpuISD::SHF, DL, Op->getValueType(0),
                       Op->getOperand(2), Op->getOperand(1));
  }
  case Intrinsic::fgpu_sldi_b:
  case Intrinsic::fgpu_sldi_h:
  case Intrinsic::fgpu_sldi_w:
  case Intrinsic::fgpu_sldi_d: {
    // Report an error for out of range values.
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::fgpu_sldi_b: Max = 15; break;
    case Intrinsic::fgpu_sldi_h: Max = 7; break;
    case Intrinsic::fgpu_sldi_w: Max = 3; break;
    case Intrinsic::fgpu_sldi_d: Max = 1; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
  ...
  case Intrinsic::fgpu_sll_b:
  case Intrinsic::fgpu_sll_h:
  case Intrinsic::fgpu_sll_w:
  case Intrinsic::fgpu_sll_d:
    return DAG.getNode(ISD::SHL, DL, Op->getValueType(0), Op->getOperand(1),
                       truncateVecElts(Op, DAG));
  case Intrinsic::fgpu_slli_b:
  case Intrinsic::fgpu_slli_h:
  case Intrinsic::fgpu_slli_w:
  case Intrinsic::fgpu_slli_d:
    return DAG.getNode(ISD::SHL, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::fgpu_splat_b:
  case Intrinsic::fgpu_splat_h:
  case Intrinsic::fgpu_splat_w:
  case Intrinsic::fgpu_splat_d:
    ...
    // EXTRACT_VECTOR_ELT can't extract i64's on FGPU32.
    // Instead we lower to FgpuISD::VSHF and match from there.
    return DAG.getNode(FgpuISD::VSHF, DL, Op->getValueType(0),
                       lowerMSASplatZExt(Op, 2, DAG), Op->getOperand(1),
                       Op->getOperand(1));
  case Intrinsic::fgpu_splati_b:
  case Intrinsic::fgpu_splati_h:
  case Intrinsic::fgpu_splati_w:
  case Intrinsic::fgpu_splati_d:
    return DAG.getNode(FgpuISD::VSHF, DL, Op->getValueType(0),
                       lowerMSASplatImm(Op, 2, DAG), Op->getOperand(1),
                       Op->getOperand(1));
  case Intrinsic::fgpu_sra_b:
  case Intrinsic::fgpu_sra_h:
  case Intrinsic::fgpu_sra_w:
  case Intrinsic::fgpu_sra_d:
    return DAG.getNode(ISD::SRA, DL, Op->getValueType(0), Op->getOperand(1),
                       truncateVecElts(Op, DAG));
  case Intrinsic::fgpu_srai_b:
  case Intrinsic::fgpu_srai_h:
  case Intrinsic::fgpu_srai_w:
  case Intrinsic::fgpu_srai_d:
    return DAG.getNode(ISD::SRA, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::fgpu_srari_b:
  case Intrinsic::fgpu_srari_h:
  case Intrinsic::fgpu_srari_w:
  case Intrinsic::fgpu_srari_d: {
    // Report an error for out of range values.
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::fgpu_srari_b: Max = 7; break;
    case Intrinsic::fgpu_srari_h: Max = 15; break;
    case Intrinsic::fgpu_srari_w: Max = 31; break;
    case Intrinsic::fgpu_srari_d: Max = 63; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > Max)
  ...
  case Intrinsic::fgpu_srl_b:
  case Intrinsic::fgpu_srl_h:
  case Intrinsic::fgpu_srl_w:
  case Intrinsic::fgpu_srl_d:
    return DAG.getNode(ISD::SRL, DL, Op->getValueType(0), Op->getOperand(1),
                       truncateVecElts(Op, DAG));
  case Intrinsic::fgpu_srli_b:
  case Intrinsic::fgpu_srli_h:
  case Intrinsic::fgpu_srli_w:
  case Intrinsic::fgpu_srli_d:
    return DAG.getNode(ISD::SRL, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::fgpu_srlri_b:
  case Intrinsic::fgpu_srlri_h:
  case Intrinsic::fgpu_srlri_w:
  case Intrinsic::fgpu_srlri_d: {
    // Report an error for out of range values.
    int64_t Max;
    switch (Intrinsic) {
    case Intrinsic::fgpu_srlri_b: Max = 7; break;
    case Intrinsic::fgpu_srlri_h: Max = 15; break;
    case Intrinsic::fgpu_srlri_w: Max = 31; break;
    case Intrinsic::fgpu_srlri_d: Max = 63; break;
    default: llvm_unreachable("Unmatched intrinsic");
    }
    int64_t Value = cast<ConstantSDNode>(Op->getOperand(2))->getSExtValue();
    if (Value < 0 || Value > Max)
  ...
  case Intrinsic::fgpu_subvi_d:
    return DAG.getNode(ISD::SUB, DL, Op->getValueType(0),
                       Op->getOperand(1), lowerMSASplatImm(Op, 2, DAG));
  case Intrinsic::fgpu_vshf_b:
  case Intrinsic::fgpu_vshf_h:
  case Intrinsic::fgpu_vshf_w:
  case Intrinsic::fgpu_vshf_d:
    return DAG.getNode(FgpuISD::VSHF, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2), Op->getOperand(3));
  case Intrinsic::fgpu_xor_v:
    return DAG.getNode(ISD::XOR, DL, Op->getValueType(0), Op->getOperand(1),
                       Op->getOperand(2));
  case Intrinsic::fgpu_xori_b:
...
SDValue FgpuSETargetLowering::lowerINTRINSIC_W_CHAIN(SDValue Op,
  ...
  default:
    return SDValue();
  case Intrinsic::fgpu_extp:
    return lowerDSPIntr(Op, DAG, FgpuISD::EXTP);
  case Intrinsic::fgpu_extpdp:
    return lowerDSPIntr(Op, DAG, FgpuISD::EXTPDP);
  case Intrinsic::fgpu_extr_w:
    return lowerDSPIntr(Op, DAG, FgpuISD::EXTR_W);
  case Intrinsic::fgpu_extr_r_w:
    return lowerDSPIntr(Op, DAG, FgpuISD::EXTR_R_W);
  case Intrinsic::fgpu_extr_rs_w:
    return lowerDSPIntr(Op, DAG, FgpuISD::EXTR_RS_W);
  case Intrinsic::fgpu_extr_s_h:
    return lowerDSPIntr(Op, DAG, FgpuISD::EXTR_S_H);
  case Intrinsic::fgpu_mthlip:
    return lowerDSPIntr(Op, DAG, FgpuISD::MTHLIP);
  case Intrinsic::fgpu_mulsaq_s_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::MULSAQ_S_W_PH);
  case Intrinsic::fgpu_maq_s_w_phl:
    return lowerDSPIntr(Op, DAG, FgpuISD::MAQ_S_W_PHL);
  case Intrinsic::fgpu_maq_s_w_phr:
    return lowerDSPIntr(Op, DAG, FgpuISD::MAQ_S_W_PHR);
  case Intrinsic::fgpu_maq_sa_w_phl:
    return lowerDSPIntr(Op, DAG, FgpuISD::MAQ_SA_W_PHL);
  case Intrinsic::fgpu_maq_sa_w_phr:
    return lowerDSPIntr(Op, DAG, FgpuISD::MAQ_SA_W_PHR);
  case Intrinsic::fgpu_dpaq_s_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPAQ_S_W_PH);
  case Intrinsic::fgpu_dpsq_s_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPSQ_S_W_PH);
  case Intrinsic::fgpu_dpaq_sa_l_w:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPAQ_SA_L_W);
  case Intrinsic::fgpu_dpsq_sa_l_w:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPSQ_SA_L_W);
  case Intrinsic::fgpu_dpaqx_s_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPAQX_S_W_PH);
  case Intrinsic::fgpu_dpaqx_sa_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPAQX_SA_W_PH);
  case Intrinsic::fgpu_dpsqx_s_w_ph:
    return lowerDSPIntr(Op, DAG, FgpuISD::DPSQX_S_W_PH);
...
static SDValue lowerVECTOR_SHUFFLE_SHF(SDValue Op, EVT ResTy,
  ...
  SDLoc DL(Op);
  return DAG.getNode(FgpuISD::SHF, DL, ResTy,
...
static SDValue lowerVECTOR_SHUFFLE_ILVEV(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(FgpuISD::ILVEV, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_ILVOD(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(FgpuISD::ILVOD, SDLoc(Op), ResTy, Wt, Ws);
}
...
static SDValue lowerVECTOR_SHUFFLE_ILVR(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(FgpuISD::ILVR, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_ILVL(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(FgpuISD::ILVL, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_PCKEV(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(FgpuISD::PCKEV, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_PCKOD(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(FgpuISD::PCKOD, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_VSHF(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(FgpuISD::VSHF, DL, ResTy, MaskVec, Op1, Op0);
}

// Lower VECTOR_SHUFFLE into one of a number of instructions depending on the
// indices in the shuffle.
SDValue FgpuSETargetLowering::lowerVECTOR_SHUFFLE(SDValue Op,
                                                  SelectionDAG &DAG) const {
...
MachineBasicBlock *
FgpuSETargetLowering::emitBPOSGE32(MachineInstr &MI,
                                   MachineBasicBlock *BB) const {
  // $bb:
  //  bposge32_pseudo $vr0
  //  =>
  ...
  BuildMI(*FBB, FBB->end(), DL, TII->get(Fgpu::ADDiu), VR2)
    .addReg(Fgpu::ZERO).addImm(0);
  BuildMI(*FBB, FBB->end(), DL, TII->get(Fgpu::B)).addMBB(Sink);

  // Fill $TBB.
  Register VR1 = RegInfo.createVirtualRegister(RC);
  BuildMI(*TBB, TBB->end(), DL, TII->get(Fgpu::ADDiu), VR1)
    .addReg(Fgpu::ZERO).addImm(1);

  // Insert phi function to $Sink.
  BuildMI(*Sink, Sink->begin(), DL, TII->get(Fgpu::PHI),
...
MachineBasicBlock *FgpuSETargetLowering::emitMSACBranchPseudo(
  ...
  BuildMI(*FBB, FBB->end(), DL, TII->get(Fgpu::ADDiu), RD1)
    .addReg(Fgpu::ZERO).addImm(0);
  BuildMI(*FBB, FBB->end(), DL, TII->get(Fgpu::B)).addMBB(Sink);

  // Fill $TBB.
  Register RD2 = RegInfo.createVirtualRegister(RC);
  BuildMI(*TBB, TBB->end(), DL, TII->get(Fgpu::ADDiu), RD2)
    .addReg(Fgpu::ZERO).addImm(1);
...
MachineBasicBlock *
FgpuSETargetLowering::emitCOPY_FW(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  if (Lane == 0) {
    ...
    if (!Subtarget.useOddSPReg()) {
      ...
      Wt = RegInfo.createVirtualRegister(&Fgpu::MSA128WEvensRegClass);

      BuildMI(*BB, MI, DL, TII->get(Fgpu::COPY), Wt).addReg(Ws);
    }

    BuildMI(*BB, MI, DL, TII->get(Fgpu::COPY), Fd).addReg(Wt, 0, Fgpu::sub_lo);
  } else {
    Register Wt = RegInfo.createVirtualRegister(
        Subtarget.useOddSPReg() ? &Fgpu::MSA128WRegClass
                                : &Fgpu::MSA128WEvensRegClass);

    BuildMI(*BB, MI, DL, TII->get(Fgpu::SPLATI_W), Wt).addReg(Ws).addImm(Lane);
    BuildMI(*BB, MI, DL, TII->get(Fgpu::COPY), Fd).addReg(Wt, 0, Fgpu::sub_lo);
  }
...
MachineBasicBlock *
FgpuSETargetLowering::emitCOPY_FD(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  ...
  if (Lane == 0)
    BuildMI(*BB, MI, DL, TII->get(Fgpu::COPY), Fd).addReg(Ws, 0, Fgpu::sub_64);
  else {
    Register Wt = RegInfo.createVirtualRegister(&Fgpu::MSA128DRegClass);

    BuildMI(*BB, MI, DL, TII->get(Fgpu::SPLATI_D), Wt).addReg(Ws).addImm(1);
    BuildMI(*BB, MI, DL, TII->get(Fgpu::COPY), Fd).addReg(Wt, 0, Fgpu::sub_64);
  }
...
MachineBasicBlock *
FgpuSETargetLowering::emitINSERT_FW(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  BuildMI(*BB, MI, DL, TII->get(Fgpu::SUBREG_TO_REG), Wt)
      .addImm(0)
      .addReg(Fs)
      .addImm(Fgpu::sub_lo);
  BuildMI(*BB, MI, DL, TII->get(Fgpu::INSVE_W), Wd)
      .addReg(Wd_in)
      .addImm(Lane)
      .addReg(Wt)
      .addImm(0);
...
MachineBasicBlock *
FgpuSETargetLowering::emitINSERT_FD(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  assert(Subtarget.isFP64bit());
  ...
  BuildMI(*BB, MI, DL, TII->get(Fgpu::SUBREG_TO_REG), Wt)
      ...
      .addReg(Fs)
      .addImm(Fgpu::sub_64);
  BuildMI(*BB, MI, DL, TII->get(Fgpu::INSVE_D), Wd)
      .addReg(Wd_in)
      .addImm(Lane)
      .addReg(Wt)
      .addImm(0);
...
// (NEG $lanetmp2, $lanetmp1)
// (SLD_B $wd, $wdtmp2, $wdtmp2,  $lanetmp2)
MachineBasicBlock *FgpuSETargetLowering::emitINSERT_DF_VIDX(
  ...
  const TargetRegisterClass *GPRRC =
      Subtarget.isABI_N64() ? &Fgpu::GPR64RegClass : &Fgpu::GPR32RegClass;
  unsigned SubRegIdx = Subtarget.isABI_N64() ? Fgpu::sub_32 : 0;
  unsigned ShiftOp = Subtarget.isABI_N64() ? Fgpu::DSLL : Fgpu::SLL;
  ...
  case 1:
    EltLog2Size = 0;
    InsertOp = Fgpu::INSERT_B;
    InsveOp = Fgpu::INSVE_B;
    VecRC = &Fgpu::MSA128BRegClass;
    break;
  case 2:
    EltLog2Size = 1;
    InsertOp = Fgpu::INSERT_H;
    InsveOp = Fgpu::INSVE_H;
    VecRC = &Fgpu::MSA128HRegClass;
    break;
  case 4:
    EltLog2Size = 2;
    InsertOp = Fgpu::INSERT_W;
    InsveOp = Fgpu::INSVE_W;
    VecRC = &Fgpu::MSA128WRegClass;
    break;
  case 8:
    EltLog2Size = 3;
    InsertOp = Fgpu::INSERT_D;
    InsveOp = Fgpu::INSVE_D;
    VecRC = &Fgpu::MSA128DRegClass;
    break;
  }
  ...
  if (IsFP) {
    ...
    BuildMI(*BB, MI, DL, TII->get(Fgpu::SUBREG_TO_REG), Wt)
        .addImm(0)
        .addReg(SrcValReg)
        .addImm(EltSizeInBytes == 8 ? Fgpu::sub_64 : Fgpu::sub_lo);
    SrcValReg = Wt;
  }
  ...
  BuildMI(*BB, MI, DL, TII->get(Fgpu::SLD_B), WdTmp1)
      .addReg(SrcVecReg)
      .addReg(SrcVecReg)
  ...
  BuildMI(*BB, MI, DL, TII->get(Subtarget.isABI_N64() ? Fgpu::DSUB : Fgpu::SUB),
          LaneTmp2)
      .addReg(Subtarget.isABI_N64() ? Fgpu::ZERO_64 : Fgpu::ZERO)
      .addReg(LaneReg);
  BuildMI(*BB, MI, DL, TII->get(Fgpu::SLD_B), Wd)
      .addReg(WdTmp2)
      .addReg(WdTmp2)
...
// splati.w $wd, $wt2[0]
MachineBasicBlock *
FgpuSETargetLowering::emitFILL_FW(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  BuildMI(*BB, MI, DL, TII->get(Fgpu::IMPLICIT_DEF), Wt1);
  BuildMI(*BB, MI, DL, TII->get(Fgpu::INSERT_SUBREG), Wt2)
      .addReg(Wt1)
      .addReg(Fs)
      .addImm(Fgpu::sub_lo);
  BuildMI(*BB, MI, DL, TII->get(Fgpu::SPLATI_W), Wd).addReg(Wt2).addImm(0);
...
// splati.d $wd, $wt2[0]
MachineBasicBlock *
FgpuSETargetLowering::emitFILL_FD(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  assert(Subtarget.isFP64bit());
  ...
  Register Wd = MI.getOperand(0).getReg();
  Register Fs = MI.getOperand(1).getReg();
  Register Wt1 = RegInfo.createVirtualRegister(&Fgpu::MSA128DRegClass);
  Register Wt2 = RegInfo.createVirtualRegister(&Fgpu::MSA128DRegClass);
  ...
  BuildMI(*BB, MI, DL, TII->get(Fgpu::INSERT_SUBREG), Wt2)
      ...
      .addReg(Fs)
      .addImm(Fgpu::sub_64);
  BuildMI(*BB, MI, DL, TII->get(Fgpu::SPLATI_D), Wd).addReg(Wt2).addImm(0);
...
// space.
MachineBasicBlock *
FgpuSETargetLowering::emitST_F16_PSEUDO(MachineInstr &MI,
                                       MachineBasicBlock *BB) const {
  ...
  const TargetRegisterClass *RC =
      MI.getOperand(1).isReg() ? RegInfo.getRegClass(MI.getOperand(1).getReg())
                               : (Subtarget.isABI_O32() ? &Fgpu::GPR32RegClass
                                                        : &Fgpu::GPR64RegClass);
  const bool UsingFgpu32 = RC == &Fgpu::GPR32RegClass;
  Register Rs = RegInfo.createVirtualRegister(&Fgpu::GPR32RegClass);

  BuildMI(*BB, MI, DL, TII->get(Fgpu::COPY_U_H), Rs).addReg(Ws).addImm(0);
  if(!UsingFgpu32) {
    Register Tmp = RegInfo.createVirtualRegister(&Fgpu::GPR64RegClass);
    BuildMI(*BB, MI, DL, TII->get(Fgpu::SUBREG_TO_REG), Tmp)
        .addImm(0)
        .addReg(Rs)
        .addImm(Fgpu::sub_32);
    Rs = Tmp;
  }
  BuildMI(*BB, MI, DL, TII->get(UsingFgpu32 ? Fgpu::SH : Fgpu::SH64))
      .addReg(Rs)
      .addReg(Rt)
      .addImm(Imm)
...
//     intervention.
MachineBasicBlock *
FgpuSETargetLowering::emitLD_F16_PSEUDO(MachineInstr &MI,
                                       MachineBasicBlock *BB) const {
  ...
  MachineInstrBuilder MIB =
      BuildMI(*BB, MI, DL, TII->get(UsingFgpu32 ? Fgpu::LH : Fgpu::LH64), Rt);
  for (unsigned i = 1; i < MI.getNumOperands(); i++)
    MIB.add(MI.getOperand(i));

  if(!UsingFgpu32) {
    Register Tmp = RegInfo.createVirtualRegister(&Fgpu::GPR32RegClass);
    BuildMI(*BB, MI, DL, TII->get(Fgpu::COPY), Tmp).addReg(Rt, 0, Fgpu::sub_32);
    Rt = Tmp;
  }

  BuildMI(*BB, MI, DL, TII->get(Fgpu::FILL_H), Wd).addReg(Rt);
...
MachineBasicBlock *
FgpuSETargetLowering::emitFPROUND_PSEUDO(MachineInstr &MI,
                                         MachineBasicBlock *BB,
                                         bool IsFGR64) const {

  // Strictly speaking, we need FGPU32R5 to support MSA. We'll be generous
  // here. It's technically doable to support FGPU32 here, but the ISA forbids
  // it.
  assert(Subtarget.hasMSA() && Subtarget.hasFgpu32r2());

  bool IsFGR64onFgpu64 = Subtarget.hasFgpu64() && IsFGR64;
  bool IsFGR64onFgpu32 = !Subtarget.hasFgpu64() && IsFGR64;
  ...
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  Register Wtemp = RegInfo.createVirtualRegister(&Fgpu::MSA128WRegClass);
  const TargetRegisterClass *GPRRC =
      IsFGR64onFgpu64 ? &Fgpu::GPR64RegClass : &Fgpu::GPR32RegClass;
  unsigned MFC1Opc = IsFGR64onFgpu64
                         ? Fgpu::DMFC1
                         : (IsFGR64onFgpu32 ? Fgpu::MFC1_D64 : Fgpu::MFC1);
  unsigned FILLOpc = IsFGR64onFgpu64 ? Fgpu::FILL_D : Fgpu::FILL_W;
  ...
  if (IsFGR64onFgpu32) {
    ...
    BuildMI(*BB, MI, DL, TII->get(Fgpu::MFHC1_D64), Rtemp2).addReg(Fs);
    Register Wtemp2 = RegInfo.createVirtualRegister(&Fgpu::MSA128WRegClass);
    Register Wtemp3 = RegInfo.createVirtualRegister(&Fgpu::MSA128WRegClass);
    BuildMI(*BB, MI, DL, TII->get(Fgpu::INSERT_W), Wtemp2)
        .addReg(Wtemp)
        .addReg(Rtemp2)
        .addImm(1);
    BuildMI(*BB, MI, DL, TII->get(Fgpu::INSERT_W), Wtemp3)
        .addReg(Wtemp2)
        .addReg(Rtemp2)
        .addImm(3);
    WPHI = Wtemp3;
  ...
  if (IsFGR64) {
    Register Wtemp2 = RegInfo.createVirtualRegister(&Fgpu::MSA128WRegClass);
    BuildMI(*BB, MI, DL, TII->get(Fgpu::FEXDO_W), Wtemp2)
        .addReg(WPHI)
        .addReg(WPHI);
    WPHI = Wtemp2;
  }

  BuildMI(*BB, MI, DL, TII->get(Fgpu::FEXDO_H), Wd).addReg(WPHI).addReg(WPHI);
...
MachineBasicBlock *
FgpuSETargetLowering::emitFPEXTEND_PSEUDO(MachineInstr &MI,
                                          MachineBasicBlock *BB,
                                          bool IsFGR64) const {
  ...
  const TargetRegisterClass *GPRRC =
      IsFGR64onFgpu64 ? &Fgpu::GPR64RegClass : &Fgpu::GPR32RegClass;
  unsigned MTC1Opc = IsFGR64onFgpu64
                         ? Fgpu::DMTC1
                         : (IsFGR64onFgpu32 ? Fgpu::MTC1_D64 : Fgpu::MTC1);
  Register COPYOpc = IsFGR64onFgpu64 ? Fgpu::COPY_S_D : Fgpu::COPY_S_W;

  Register Wtemp = RegInfo.createVirtualRegister(&Fgpu::MSA128WRegClass);
  Register WPHI = Wtemp;

  BuildMI(*BB, MI, DL, TII->get(Fgpu::FEXUPR_W), Wtemp).addReg(Ws);
  if (IsFGR64) {
    WPHI = RegInfo.createVirtualRegister(&Fgpu::MSA128DRegClass);
    BuildMI(*BB, MI, DL, TII->get(Fgpu::FEXUPR_D), WPHI).addReg(Wtemp);
  }

  // Perform the safety regclass copy mentioned above.
  Register Rtemp = RegInfo.createVirtualRegister(GPRRC);
  Register FPRPHI = IsFGR64onFgpu32
                        ? RegInfo.createVirtualRegister(&Fgpu::FGR64RegClass)
                        : Fd;
  ...
  if (IsFGR64onFgpu32) {
    Register Rtemp2 = RegInfo.createVirtualRegister(GPRRC);
    BuildMI(*BB, MI, DL, TII->get(Fgpu::COPY_S_W), Rtemp2)
        .addReg(WPHI)
        .addImm(1);
    BuildMI(*BB, MI, DL, TII->get(Fgpu::MTHC1_D64), Fd)
        .addReg(FPRPHI)
        .addReg(Rtemp2);
  }
...
// fexp2.w $wd, $ws, $wt
MachineBasicBlock *
FgpuSETargetLowering::emitFEXP2_W_1(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  const TargetRegisterClass *RC = &Fgpu::MSA128WRegClass;
  Register Ws1 = RegInfo.createVirtualRegister(RC);
  Register Ws2 = RegInfo.createVirtualRegister(RC);
  DebugLoc DL = MI.getDebugLoc();

  // Splat 1.0 into a vector
  BuildMI(*BB, MI, DL, TII->get(Fgpu::LDI_W), Ws1).addImm(1);
  BuildMI(*BB, MI, DL, TII->get(Fgpu::FFINT_U_W), Ws2).addReg(Ws1);

  // Emit 1.0 * fexp2(Wt)
  BuildMI(*BB, MI, DL, TII->get(Fgpu::FEXP2_W), MI.getOperand(0).getReg())
      .addReg(Ws2)
      .addReg(MI.getOperand(1).getReg());
...
// fexp2.d $wd, $ws, $wt
MachineBasicBlock *
FgpuSETargetLowering::emitFEXP2_D_1(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  const TargetRegisterClass *RC = &Fgpu::MSA128DRegClass;
  Register Ws1 = RegInfo.createVirtualRegister(RC);
  Register Ws2 = RegInfo.createVirtualRegister(RC);
  DebugLoc DL = MI.getDebugLoc();

  // Splat 1.0 into a vector
  BuildMI(*BB, MI, DL, TII->get(Fgpu::LDI_D), Ws1).addImm(1);
  BuildMI(*BB, MI, DL, TII->get(Fgpu::FFINT_U_D), Ws2).addReg(Ws1);

  // Emit 1.0 * fexp2(Wt)
  BuildMI(*BB, MI, DL, TII->get(Fgpu::FEXP2_D), MI.getOperand(0).getReg())
      .addReg(Ws2)
      .addReg(MI.getOperand(1).getReg());
