//===- FgpuConstantIslandPass.cpp - Emit Pc Relative loads ----------------===//
//
...
// For now, only Fgpu16 will use this.
//
// Loading constants inline is expensive on Fgpu16 and it's in general better
...
#define DEBUG_TYPE "fgpu-constant-islands"
...
static cl::opt<bool>
AlignConstantIslands("fgpu-align-constant-islands", cl::Hidden, cl::init(true),
...
static cl::opt<int> ConstantIslandsSmallOffset(
  "fgpu-constant-islands-small-offset",
  cl::init(0),
...
static cl::opt<bool> NoLoadRelaxation(
  "fgpu-constant-islands-no-load-relaxation",
  cl::init(false),
...
static unsigned int branchTargetOperand(MachineInstr *MI) {
  ...
  case Fgpu::JalB16:
    return 0;
  case Fgpu::BeqzRxImm16:
  case Fgpu::BeqzRxImmX16:
  case Fgpu::BnezRxImm16:
  case Fgpu::BnezRxImmX16:
    return 1;
  }
...
static unsigned int longformBranchOpcode(unsigned int Opcode) {
  ...
  case Fgpu::Bteqz16:
  case Fgpu::BteqzX16:
    return Fgpu::BteqzX16;
  case Fgpu::Btnez16:
  case Fgpu::BtnezX16:
    return Fgpu::BtnezX16;
  case Fgpu::JalB16:
    return Fgpu::JalB16;
  case Fgpu::BeqzRxImm16:
  case Fgpu::BeqzRxImmX16:
    return Fgpu::BeqzRxImmX16;
  case Fgpu::BnezRxImm16:
  case Fgpu::BnezRxImmX16:
    return Fgpu::BnezRxImmX16;
  }
  llvm_unreachable("Unknown branch type");
}
...
namespace {
  ...
  /// FgpuConstantIslands - Due to limited PC-relative displacements, Fgpu
  ...
  class FgpuConstantIslands : public MachineFunctionPass {
    ...
    private:
      unsigned MaxDisp;
      unsigned LongFormMaxDisp; // fgpu16 has 16/32 bit instructions
  ...
  const FgpuSubtarget *STI = nullptr;
  const Fgpu16InstrInfo *TII;
  ...
  public:
    ...
    FgpuConstantIslands() : MachineFunctionPass(ID) {}

    StringRef getPassName() const override { return "Fgpu Constant Islands"; }
...
char FgpuConstantIslands::ID = 0;

bool FgpuConstantIslands::isOffsetInRange
...
LLVM_DUMP_METHOD void FgpuConstantIslands::dumpBBs() {
...
bool FgpuConstantIslands::runOnMachineFunction(MachineFunction &mf) {
  // The intention is for this to be a fgpu16 only pass for now
  // FIXME:
  MF = &mf;
  ...
  LLVM_DEBUG(dbgs() << "constant island machine function "
                    << "\n");
  if (!STI->inFgpu16Mode() || !FgpuSubtarget::useConstantIslands()) {
    return false;
  }
  TII = (const Fgpu16InstrInfo *)STI->getInstrInfo();
  MFI = MF->getInfo<FgpuFunctionInfo>();
...
void
FgpuConstantIslands::doInitialPlacement(std::vector<MachineInstr*> &CPEMIs) {
  ...
  for (unsigned i = 0, e = CPs.size(); i != e; ++i) {
    ...
    MachineInstr *CPEMI =
      BuildMI(*BB, InsAt, DebugLoc(), TII->get(Fgpu::CONSTPOOL_ENTRY))
...
FgpuConstantIslands::CPEntry
*FgpuConstantIslands::findConstPoolEntry(unsigned CPI,
...
Align FgpuConstantIslands::getCPEAlign(const MachineInstr &CPEMI) {
  assert(CPEMI.getOpcode() == Fgpu::CONSTPOOL_ENTRY);
    ...
    for (MachineInstr &MI : MBB) {
      ...
      for (unsigned op = 0, e = MI.getNumOperands(); op != e; ++op)
        if (MI.getOperand(op).isCPI()) {
          ...
          case Fgpu::LwRxPcTcp16:
            Bits = 8;
            Scale = 4;
            LongFormOpcode = Fgpu::LwRxPcTcpX16;
            LongFormBits = 14;
            LongFormScale = 1;
            break;
          case Fgpu::LwRxPcTcpX16:
            Bits = 14;
            Scale = 1;
            NegOk = true;
            break;
...
void FgpuConstantIslands::computeBlockSize(MachineBasicBlock *MBB) {
...
unsigned FgpuConstantIslands::getOffsetOf(MachineInstr *MI) const {
...
void FgpuConstantIslands::updateForInsertedWaterBlock
...
unsigned FgpuConstantIslands::getUserOffset(CPUser &U) const {
...
MachineBasicBlock *
FgpuConstantIslands::splitBlockBeforeInstr(MachineInstr &MI) {
  ...
  BuildMI(OrigBB, DebugLoc(), TII->get(Fgpu::Bimm16)).addMBB(NewBB);
  ++NumSplit;
...
/// constant pool entry).
bool FgpuConstantIslands::isOffsetInRange(unsigned UserOffset,
...
bool FgpuConstantIslands::isWaterInRange(unsigned UserOffset,
...
bool FgpuConstantIslands::isCPEntryInRange
...
static bool BBIsJumpedOver(MachineBasicBlock *MBB) {
  ...
  if (PredMI->getOpcode() == Fgpu::Bimm16)
    return PredMI->getOperand(0).getMBB() == Succ;
  return false;
}
#endif

void FgpuConstantIslands::adjustBBOffsetsAfter(MachineBasicBlock *BB) {
...
bool FgpuConstantIslands::decrementCPEReferenceCount(unsigned CPI,
                                                    MachineInstr *CPEMI) {
...
int FgpuConstantIslands::findInRangeCPEntry(CPUser& U, unsigned UserOffset)
{
...
/// 1 = entry found, and there were no code insertions or deletions
/// 2 = entry found, and there were code insertions or deletions
int FgpuConstantIslands::findLongFormInRangeCPEntry
...
bool FgpuConstantIslands::findAvailableWater(CPUser &U, unsigned UserOffset,
                                      water_iterator &WaterIter) {
  if (WaterList.empty())
    return false;
...
/// is not adjusted).
void FgpuConstantIslands::createNewWater(unsigned CPUserIndex,
                                        unsigned UserOffset,
                                        MachineBasicBlock *&NewMBB) {
...
bool FgpuConstantIslands::handleConstantPoolUser(unsigned CPUserIndex) {
  CPUser &U = CPUsers[CPUserIndex];
  MachineInstr *UserMI = U.MI;
  MachineInstr *CPEMI  = U.CPEMI;
  unsigned CPI = CPEMI->getOperand(1).getIndex();
  ...
  U.CPEMI = BuildMI(NewIsland, DebugLoc(), TII->get(Fgpu::CONSTPOOL_ENTRY))
...
void FgpuConstantIslands::removeDeadCPEMI(MachineInstr *CPEMI) {
...
/// are zero.
bool FgpuConstantIslands::removeUnusedCPEntries() {
...
bool FgpuConstantIslands::isBBInRange
...
bool FgpuConstantIslands::fixupImmediateBr(ImmBranch &Br) {
...
bool
FgpuConstantIslands::fixupUnconditionalBr(ImmBranch &Br) {
  MachineInstr *MI = Br.MI;
  MachineBasicBlock *MBB = MI->getParent();
  ...
  if (isBBInRange(MI, DestBB, BimmX16MaxDisp)) {
    ...
    MI->setDesc(TII->get(Fgpu::BimmX16));
  }
  else {
    ...
    Br.MaxDisp = ((1<<24)-1) * 2;
    MI->setDesc(TII->get(Fgpu::JalB16));
  }
...
bool
FgpuConstantIslands::fixupConditionalBr(ImmBranch &Br) {
  MachineInstr *MI = Br.MI;
  unsigned TargetOperand = branchTargetOperand(MI);
  MachineBasicBlock *DestBB = MI->getOperand(TargetOperand).getMBB();
...
void FgpuConstantIslands::prescanForConstants() {
  unsigned J = 0;
  (void)J;
      ...
      switch(I->getDesc().getOpcode()) {
        case Fgpu::LwConstant32: {
          ...
          if (Literal.isImm()) {
            ...
            LLVM_DEBUG(dbgs() << "constant island constant " << *I << "\n");
            I->setDesc(TII->get(Fgpu::LwRxPcTcp16));
            I->RemoveOperand(1);
            I->RemoveOperand(1);
...
FunctionPass *llvm::createFgpuConstantIslandPass() {
  return new FgpuConstantIslands();
}
