...
public:
  ...
  bool hasFgpu3() const { return STI.getFeatureBits()[Fgpu::FeatureFgpu3]; }
  bool hasFgpu32() const { return STI.getFeatureBits()[Fgpu::FeatureFgpu32]; }

  bool hasFgpu32r6() const {
    return STI.getFeatureBits()[Fgpu::FeatureFgpu32r6];
  }

  bool isFP64() const { return STI.getFeatureBits()[Fgpu::FeatureFP64Bit]; }

  bool isGP64() const { return STI.getFeatureBits()[Fgpu::FeatureGP64Bit]; }

  bool isPTR64() const { return STI.getFeatureBits()[Fgpu::FeaturePTR64Bit]; }

  bool hasCnFgpu() const { return STI.getFeatureBits()[Fgpu::FeatureCnFgpu]; }

  bool hasCnFgpuP() const { return STI.getFeatureBits()[Fgpu::FeatureCnFgpuP]; }

  bool hasCOP3() const {
    // Only present in FGPU-I and FGPU-II
    return !hasFgpu32() && !hasFgpu3();
  }
...
// DecodeBranchTarget7MM - Decode microFGPU branch offset, which is
...
// DecodeBranchTarget10MM - Decode microFGPU branch offset, which is
// shifted left by 1 bit.
...
// DecodeBranchTargetMM - Decode microFGPU branch offset, which is
// shifted left by 1 bit.
...
// DecodeBranchTarget26MM - Decode microFGPU branch offset, which is
// shifted left by 1 bit.
...
// DecodeJumpTargetMM - Decode microFGPU jump target, which is
// shifted left by 1 bit.
...
// DecodeJumpTargetXMM - Decode microFGPU jump and link exchange target,
...
static MCDisassembler *createFgpuDisassembler(
                       ...
                       MCContext &Ctx) {
  return new FgpuDisassembler(STI, Ctx, true);
}

static MCDisassembler *createFgpuelDisassembler(
                       const Target &T,
                       const MCSubtargetInfo &STI,
                       MCContext &Ctx) {
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeFgpuDisassembler() {
  ...
  TargetRegistry::RegisterMCDisassembler(getTheFgpuelTarget(),
                                         createFgpuelDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheFgpu64Target(),
                                         createFgpuDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheFgpu64elTarget(),
                                         createFgpuelDisassembler);
}
...
static DecodeStatus DecodeDAHIDATIMMR6(MCInst &MI, InsnType insn,
  ...
  MI.addOperand(MCOperand::createReg(getReg(Decoder, Fgpu::GPR64RegClassID,
                                       Rs)));
  MI.addOperand(MCOperand::createReg(getReg(Decoder, Fgpu::GPR64RegClassID,
                                       Rs)));
...
static DecodeStatus DecodeAddiGroupBranch(MCInst &MI, InsnType insn,
                                          uint64_t Address,
                                          const void *Decoder) {
  // If we are called then we can assume that FGPU32r6/FGPU64r6 is enabled
  ...
  if (Rs >= Rt) {
    MI.setOpcode(Fgpu::BOVC);
    HasRs = true;
  } else if (Rs != 0 && Rs < Rt) {
    MI.setOpcode(Fgpu::BEQC);
    HasRs = true;
  } else
    MI.setOpcode(Fgpu::BEQZALC);
...
static DecodeStatus DecodeDaddiGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rs >= Rt) {
    MI.setOpcode(Fgpu::BNVC);
    HasRs = true;
  } else if (Rs != 0 && Rs < Rt) {
    MI.setOpcode(Fgpu::BNEC);
    HasRs = true;
  } else
    MI.setOpcode(Fgpu::BNEZALC);
...
static DecodeStatus DecodeBlezlGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(Fgpu::BLEZC);
  else if (Rs == Rt)
    MI.setOpcode(Fgpu::BGEZC);
  else {
    HasRs = true;
    MI.setOpcode(Fgpu::BGEC);
  }
...
static DecodeStatus DecodeBgtzlGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(Fgpu::BGTZC);
  else if (Rs == Rt)
    MI.setOpcode(Fgpu::BLTZC);
  else {
    MI.setOpcode(Fgpu::BLTC);
    HasRs = true;
  }
...
static DecodeStatus DecodeBgtzGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0) {
    MI.setOpcode(Fgpu::BGTZ);
    HasRs = true;
  } else if (Rs == 0) {
    MI.setOpcode(Fgpu::BGTZALC);
    HasRt = true;
  } else if (Rs == Rt) {
    MI.setOpcode(Fgpu::BLTZALC);
    HasRs = true;
  } else {
    MI.setOpcode(Fgpu::BLTUC);
    HasRs = true;
    HasRt = true;
  }
...
static DecodeStatus DecodeBlezGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(Fgpu::BLEZALC);
  else if (Rs == Rt)
    MI.setOpcode(Fgpu::BGEZALC);
  else {
    HasRs = true;
    MI.setOpcode(Fgpu::BGEUC);
  }
...
static DecodeStatus DecodeDEXT(MCInst &MI, InsnType Insn, uint64_t Address,
  ...
  MI.setOpcode(Fgpu::DEXT);

  InsnType Rs = fieldFromInstruction(Insn, 21, 5);
  InsnType Rt = fieldFromInstruction(Insn, 16, 5);

  MI.addOperand(
      MCOperand::createReg(getReg(Decoder, Fgpu::GPR64RegClassID, Rt)));
  MI.addOperand(
      MCOperand::createReg(getReg(Decoder, Fgpu::GPR64RegClassID, Rs)));
...
DecodeStatus FgpuDisassembler::getInstruction(MCInst &Instr, uint64_t &Size,
  ...
  if (hasFgpu32r6() && isGP64()) {
    LLVM_DEBUG(
        dbgs() << "Trying Fgpu32r6_64r6 (GPR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableFgpu32r6_64r6_GP6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }

  if (hasFgpu32r6() && isPTR64()) {
    LLVM_DEBUG(
        dbgs() << "Trying Fgpu32r6_64r6 (PTR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableFgpu32r6_64r6_PTR6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (hasFgpu32r6()) {
    LLVM_DEBUG(dbgs() << "Trying Fgpu32r6_64r6 table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableFgpu32r6_64r632, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (hasFgpu2() && isPTR64()) {
    LLVM_DEBUG(
        dbgs() << "Trying Fgpu32r6_64r6 (PTR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableFgpu32_64_PTR6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (hasCnFgpu()) {
    LLVM_DEBUG(dbgs() << "Trying CnFgpu table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableCnFgpu32, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (hasCnFgpuP()) {
    LLVM_DEBUG(dbgs() << "Trying CnFgpuP table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableCnFgpuP32, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (isGP64()) {
    LLVM_DEBUG(dbgs() << "Trying Fgpu64 (GPR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableFgpu6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (isFP64()) {
    LLVM_DEBUG(
        dbgs() << "Trying FgpuFP64 (64 bit FPU) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableFgpuFP6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
...
static DecodeStatus DecodeGPR64RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::GPR64RegClassID, RegNo);
...
static DecodeStatus DecodeGPRMM16RegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Fgpu::GPRMM16RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeGPRMM16ZeroRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Fgpu::GPRMM16ZeroRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeGPRMM16MovePRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Fgpu::GPRMM16MovePRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeGPR32RegisterClass(MCInst &Inst,
  ...
  if (RegNo > 31)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Fgpu::GPR32RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodePtrRegisterClass(MCInst &Inst,
                                           ...
                                           uint64_t Address,
                                           const void *Decoder) {
  if (static_cast<const FgpuDisassembler *>(Decoder)->isGP64())
...
static DecodeStatus DecodeFGR64RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::FGR64RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFGR32RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::FGR32RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeCCRRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 31)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Fgpu::CCRRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFCCRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, Fgpu::FCCRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFGRCCRegisterClass(MCInst &Inst, unsigned RegNo,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::FGRCCRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128Mem(MCInst &Inst, unsigned Insn,
  ...
  Reg = getReg(Decoder, Fgpu::MSA128BRegClassID, Reg);
  Base = getReg(Decoder, Fgpu::GPR32RegClassID, Base);
...
static DecodeStatus DecodeMemMMGPImm7Lsl2(MCInst &Inst,
  ...
  Inst.addOperand(MCOperand::createReg(Reg));
  Inst.addOperand(MCOperand::createReg(Fgpu::GP));
  Inst.addOperand(MCOperand::createImm(Offset << 2));
...
static DecodeStatus DecodeFMemMMR2(MCInst &Inst, unsigned Insn,
  ...
  // swapped according to microFGPU spec.
  int Offset = SignExtend32<16>(Insn & 0xffff);
  unsigned Base = fieldFromInstruction(Insn, 16, 5);
  unsigned Reg = fieldFromInstruction(Insn, 21, 5);
...
static DecodeStatus DecodeFMem2(MCInst &Inst,
  ...
  Reg = getReg(Decoder, Fgpu::COP2RegClassID, Reg);
  Base = getReg(Decoder, Fgpu::GPR32RegClassID, Base);
...
static DecodeStatus DecodeFMem3(MCInst &Inst,
  ...
  Reg = getReg(Decoder, Fgpu::COP3RegClassID, Reg);
  Base = getReg(Decoder, Fgpu::GPR32RegClassID, Base);
...
static DecodeStatus DecodeSpecial3LlSc(MCInst &Inst,
  ...
  Rt = getReg(Decoder, Fgpu::GPR32RegClassID, Rt);
  Base = getReg(Decoder, Fgpu::GPR32RegClassID, Base);

  if(Inst.getOpcode() == Fgpu::SC_R6 || Inst.getOpcode() == Fgpu::SCD_R6){
...
static DecodeStatus DecodeHWRegsRegisterClass(MCInst &Inst,
  ...
  if (RegNo != 29)
    return  MCDisassembler::Fail;
  Inst.addOperand(MCOperand::createReg(Fgpu::HWR29));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeAFGR64RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::AFGR64RegClassID, RegNo /2);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeACC64DSPRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::ACC64DSPRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeHI32DSPRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::HI32DSPRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeLO32DSPRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::LO32DSPRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128BRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::MSA128BRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128HRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::MSA128HRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128WRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::MSA128WRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128DRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::MSA128DRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSACtrlRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::MSACtrlRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeCOP0RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::COP0RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeCOP2RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, Fgpu::COP2RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeRegListOperand(MCInst &Inst,
                                         ...
                                         uint64_t Address,
                                         const void *Decoder) {
  unsigned Regs[] = {Fgpu::S0, Fgpu::S1, Fgpu::S2, Fgpu::S3, Fgpu::S4, Fgpu::S5,
                     Fgpu::S6, Fgpu::S7, Fgpu::FP};
  unsigned RegNum;
  ...
  if (RegLst & 0x10)
    Inst.addOperand(MCOperand::createReg(Fgpu::RA));
...
static DecodeStatus DecodeRegListOperand16(MCInst &Inst, unsigned Insn,
                                           uint64_t Address,
                                           const void *Decoder) {
  unsigned Regs[] = {Fgpu::S0, Fgpu::S1, Fgpu::S2, Fgpu::S3};
  unsigned RegLst;
  switch(Inst.getOpcode()) {
  default:
...
static DecodeStatus DecodeMovePOperands(MCInst &Inst, unsigned Insn,
  ...
  unsigned RegRs;
  if (static_cast<const FgpuDisassembler*>(Decoder)->hasFgpu32r6())
...
static DecodeStatus DecodeMovePRegPair(MCInst &Inst, unsigned RegPair,
  ...
  default:
    return MCDisassembler::Fail;
  case 0:
    Inst.addOperand(MCOperand::createReg(Fgpu::A1));
    Inst.addOperand(MCOperand::createReg(Fgpu::A2));
    break;
  case 1:
    Inst.addOperand(MCOperand::createReg(Fgpu::A1));
    Inst.addOperand(MCOperand::createReg(Fgpu::A3));
    break;
  case 2:
    Inst.addOperand(MCOperand::createReg(Fgpu::A2));
    Inst.addOperand(MCOperand::createReg(Fgpu::A3));
    break;
  case 3:
    Inst.addOperand(MCOperand::createReg(Fgpu::A0));
    Inst.addOperand(MCOperand::createReg(Fgpu::S5));
    break;
  case 4:
    Inst.addOperand(MCOperand::createReg(Fgpu::A0));
    Inst.addOperand(MCOperand::createReg(Fgpu::S6));
    break;
  case 5:
    Inst.addOperand(MCOperand::createReg(Fgpu::A0));
    Inst.addOperand(MCOperand::createReg(Fgpu::A1));
