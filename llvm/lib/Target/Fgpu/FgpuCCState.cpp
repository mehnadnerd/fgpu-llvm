//===---- FgpuCCState.cpp - CCState with Fgpu specific extensions ---------===//
//
...
bool FgpuCCState::isF128SoftLibCall(const char *CallSym) {
...
/// originally a fp128.
bool FgpuCCState::originalTypeIsF128(const Type *Ty, const char *Func) {
  if (Ty->isFP128Ty())
    return true;
...
bool FgpuCCState::originalEVTTypeIsVectorFloat(EVT Ty) {
...
bool FgpuCCState::originalTypeIsVectorFloat(const Type *Ty) {
...
FgpuCCState::SpecialCallingConvType
...
void FgpuCCState::PreAnalyzeCallResultForF128(
...
/// record this for use by RetCC_FgpuN.
void FgpuCCState::PreAnalyzeReturnForF128(
...
/// this.
void FgpuCCState::PreAnalyzeCallResultForVectorFloat(
...
/// this.
void FgpuCCState::PreAnalyzeReturnForVectorFloat(
    const SmallVectorImpl<ISD::OutputArg> &Outs) {
  for (unsigned i = 0; i < Outs.size(); ++i) {
...
void FgpuCCState::PreAnalyzeReturnValue(EVT ArgVT) {
  OriginalRetWasFloatVector.push_back(originalEVTTypeIsVectorFloat(ArgVT));
}

void FgpuCCState::PreAnalyzeCallOperand(const Type *ArgTy, bool IsFixed,
                                        const char *Func) {
...
void FgpuCCState::PreAnalyzeCallOperands(
    const SmallVectorImpl<ISD::OutputArg> &Outs,
...
void FgpuCCState::PreAnalyzeFormalArgument(const Type *ArgTy,
...
/// and record this.
void FgpuCCState::PreAnalyzeFormalArgumentsForF128(
    const SmallVectorImpl<ISD::InputArg> &Ins) {
  const MachineFunction &MF = getMachineFunction();
  for (unsigned i = 0; i < Ins.size(); ++i) {
