...
bool
FgpuAsmParser::MatchAndEmitInstruction(SMLoc IDLoc,
				       unsigned &Opcode,
				       OperandVector &Operands,
				       MCStreamer &Out,
				       uint64_t &ErrorInfo,
...
int FgpuAsmParser::matchRegisterName(StringRef Name) {
   ...
   int CC;
    ...
    if(CC == -1 and Name[0]=='r' and Name[1]=='f') {
      //llvm::errs() << "register name = " << Name << "\n";
      //llvm::errs() << "reg id = " << Name.substr(2) << "\n";
      //assert(false);
      CC = Fgpu::F0 + atoi(reinterpret_cast<const char*>(Name.substr(2).bytes_begin()));
    }
...
bool FgpuAsmParser::ParseOperand(OperandVector &Operands, StringRef Mnemonic) {
  ...
  switch (tokenType)
    ...
    default:
      ...
      /* dbsheffi : this is a horrible hack */
    case AsmToken::LBrac: {
      SMLoc S,E;
      SMLoc SS,EE;
      SS = S = Parser.getTok().getLoc();
