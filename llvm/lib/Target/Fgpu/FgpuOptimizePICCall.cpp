//===- FgpuOptimizePICCall.cpp - Optimize PIC Calls -----------------------===//
//
...
#define DEBUG_TYPE "optimize-fgpu-pic-call"

static cl::opt<bool> LoadTargetFromGOT("fgpu-load-target-from-got",
                                       cl::init(true),
                                       cl::desc("Load target address from GOT"),
                                       cl::Hidden);

static cl::opt<bool> EraseGPOpnd("fgpu-erase-gp-opnd",
...
public:
  ...
  StringRef getPassName() const override { return "Fgpu OptimizePICCall"; }
...
bool OptimizePICCall::isCallViaRegister(MachineInstr &MI, unsigned &Reg,
  ...
  if (Flags != FgpuII::MO_GOT_CALL && Flags != FgpuII::MO_CALL_LO16)
    return true;
...
FunctionPass *llvm::createFgpuOptimizePICCallPass() {
