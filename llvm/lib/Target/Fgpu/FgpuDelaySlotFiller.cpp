//===- FgpuDelaySlotFiller.cpp - Fgpu Delay Slot Filler -------------------===//
//
...
#define DEBUG_TYPE "Fgpu-delay-slot-filler"
...
static cl::opt<bool> DisableDelaySlotFiller(
  "disable-Fgpu-delay-filler",
  cl::init(false),
  cl::desc("Fill all delay slots with NOPs."),
  cl::Hidden);

static cl::opt<bool> DisableForwardSearch(
  "disable-Fgpu-df-forward-search",
  cl::init(true),
  cl::desc("Disallow Fgpu delay filler to search forward."),
  cl::Hidden);

static cl::opt<bool> DisableSuccBBSearch(
  "disable-Fgpu-df-succbb-search",
  cl::init(true),
  cl::desc("Disallow Fgpu delay filler to search successor basic blocks."),
  cl::Hidden);

static cl::opt<bool> DisableBackwardSearch(
  "disable-Fgpu-df-backward-search",
  cl::init(false),
  cl::desc("Disallow Fgpu delay filler to search backward."),
  cl::Hidden);
...
static cl::opt<CompactBranchPolicy> FgpuCompactBranchPolicy(
    "Fgpu-compact-branches", cl::Optional, cl::init(CB_Optimal),
    cl::desc("Fgpu Specific: Compact branch policy."),
...
namespace {
  ...
  class FgpuDelaySlotFiller : public MachineFunctionPass {
  public:
    FgpuDelaySlotFiller() : MachineFunctionPass(ID) {
      initializeFgpuDelaySlotFillerPass(*PassRegistry::getPassRegistry());
    }

    StringRef getPassName() const override { return "Fgpu Delay Slot Filler"; }
  ...
  private:
    ...
    std::pair<FgpuInstrInfo::BranchType, MachineInstr *>
...
char FgpuDelaySlotFiller::ID = 0;

static bool hasUnoccupiedSlot(const MachineInstr *MI) {
  return MI->hasDelaySlot() && !MI->isBundledWithSucc();
}

INITIALIZE_PASS(FgpuDelaySlotFiller, DEBUG_TYPE,
                "Fill delay slot for Fgpu", false, false)
...
void RegDefsUses::setCallerSaved(const MachineInstr &MI) {
  ...
  if (MI.definesRegister(Fgpu::RA) || MI.definesRegister(Fgpu::RA_64)) {
    Defs.set(Fgpu::RA);
    Defs.set(Fgpu::RA_64);
  }
  ...
  CallerSavedRegs.reset(Fgpu::ZERO);
  CallerSavedRegs.reset(Fgpu::ZERO_64);
...
void RegDefsUses::setUnallocatableRegs(const MachineFunction &MF) {
  ...
  AllocSet.set(Fgpu::ZERO);
  AllocSet.set(Fgpu::ZERO_64);
...
Iter FgpuDelaySlotFiller::replaceWithCompactBranch(MachineBasicBlock &MBB,
                                                   Iter Branch,
                                                   const DebugLoc &DL) {
  const FgpuSubtarget &STI = MBB.getParent()->getSubtarget<FgpuSubtarget>();
...
static int getEquivalentCallShort(int Opcode) {
  switch (Opcode) {
  case Fgpu::BGEZAL:
    return Fgpu::BGEZALS_MM;
  case Fgpu::BLTZAL:
    return Fgpu::BLTZALS_MM;
  case Fgpu::JAL:
  case Fgpu::JAL_MM:
    return Fgpu::JALS_MM;
  case Fgpu::JALR:
    return Fgpu::JALRS_MM;
  case Fgpu::JALR16_MM:
    return Fgpu::JALRS16_MM;
  case Fgpu::TAILCALL_MM:
    llvm_unreachable("Attempting to shorten the TAILCALL_MM pseudo!");
  case Fgpu::TAILCALLREG:
    return Fgpu::JR16_MM;
  default:
    llvm_unreachable("Unexpected call instruction for microFgpu.");
  }
}

/// runOnMachineBasicBlock - Fill in delay slots for the given basic block.
/// We assume there is only one delay slot per delayed instruction.
bool FgpuDelaySlotFiller::runOnMachineBasicBlock(MachineBasicBlock &MBB) {
  bool Changed = false;
  const FgpuSubtarget &STI = MBB.getParent()->getSubtarget<FgpuSubtarget>();
  bool InMicroFgpuMode = STI.inMicroFgpuMode();
  const FgpuInstrInfo *TII = STI.getInstrInfo();
  ...
  for (Iter I = MBB.begin(); I != MBB.end(); ++I) {
    ...
    // Delay slot filling is disabled at -O0, or in microFgpu32R6.
    if (!DisableDelaySlotFiller && (TM->getOptLevel() != CodeGenOpt::None) &&
        !(InMicroFgpuMode && STI.hasFgpu32r6())) {

      bool Filled = false;

      if (FgpuCompactBranchPolicy.getValue() != CB_Always ||
      ...
      if (Filled) {
        ...
        if (InMicroFgpuMode && TII->getInstSizeInBytes(*std::next(DSI)) == 2 &&
            DSI->isCall()) {
    ...
    // For microFgpu if instruction is BEQ or BNE with one ZERO register, then
    ...
    // For FgpuR6 attempt to produce the corresponding compact (no delay slot)
    ...
    if ((InMicroFgpuMode ||
         (STI.hasFgpu32r6() && FgpuCompactBranchPolicy != CB_Never)) &&
        TII->getEquivalentCompactForm(I)) {
    ...
    LLVM_DEBUG(dbgs() << DEBUG_TYPE << ": could not fill delay slot for ";
               I->dump());
    BuildMI(MBB, std::next(I), I->getDebugLoc(), TII->get(Fgpu::NOP));
...
template <typename IterTy>
bool FgpuDelaySlotFiller::searchRange(MachineBasicBlock &MBB, IterTy Begin,
  ...
  for (IterTy I = Begin; I != End;) {
    ...
    if (STI.isTargetNaCl()) {
      ...
      if ((isBasePlusOffsetMemoryAccess(CurrI->getOpcode(), &AddrIdx) &&
          ...
          CurrI->modifiesRegister(Fgpu::SP, STI.getRegisterInfo()))
        continue;
    }
    ...
    // but we don't have enough information to make that decision.
     if (InMicroFgpuMode && TII->getInstSizeInBytes(*CurrI) == 2 &&
        (Opcode == Fgpu::JR || Opcode == Fgpu::PseudoIndirectBranch ||
         Opcode == Fgpu::PseudoIndirectBranch_MM ||
         Opcode == Fgpu::PseudoReturn || Opcode == Fgpu::TAILCALL))
      continue;
     // Instructions LWP/SWP and MOVEP should not be in a delay slot as that
     // results in unpredictable behaviour
     if (InMicroFgpuMode && (Opcode == Fgpu::LWP_MM || Opcode == Fgpu::SWP_MM ||
                             Opcode == Fgpu::MOVEP_MM))
       continue;
...
bool FgpuDelaySlotFiller::searchBackward(MachineBasicBlock &MBB,
...
bool FgpuDelaySlotFiller::searchForward(MachineBasicBlock &MBB,
                                        Iter Slot) const {
...
bool FgpuDelaySlotFiller::searchSuccBBs(MachineBasicBlock &MBB,
                                        Iter Slot) const {
...
MachineBasicBlock *
FgpuDelaySlotFiller::selectSuccBB(MachineBasicBlock &B) const {
  if (B.succ_empty())
    return nullptr;
...
std::pair<FgpuInstrInfo::BranchType, MachineInstr *>
FgpuDelaySlotFiller::getBranch(MachineBasicBlock &MBB,
                               const MachineBasicBlock &Dst) const {
  const FgpuInstrInfo *TII =
      MBB.getParent()->getSubtarget<FgpuSubtarget>().getInstrInfo();
  MachineBasicBlock *TrueBB = nullptr, *FalseBB = nullptr;
  SmallVector<MachineInstr*, 2> BranchInstrs;
  SmallVector<MachineOperand, 2> Cond;

  FgpuInstrInfo::BranchType R =
      TII->analyzeBranch(MBB, TrueBB, FalseBB, Cond, false, BranchInstrs);

  if ((R == FgpuInstrInfo::BT_None) || (R == FgpuInstrInfo::BT_NoBranch))
    return std::make_pair(R, nullptr);

  if (R != FgpuInstrInfo::BT_CondUncond) {
    if (!hasUnoccupiedSlot(BranchInstrs[0]))
      return std::make_pair(FgpuInstrInfo::BT_None, nullptr);

    assert(((R != FgpuInstrInfo::BT_Uncond) || (TrueBB == &Dst)));
  ...
  if (hasUnoccupiedSlot(BranchInstrs[0]))
    return std::make_pair(FgpuInstrInfo::BT_Cond, BranchInstrs[0]);

  // If that fails, try the unconditional branch.
  if (hasUnoccupiedSlot(BranchInstrs[1]) && (FalseBB == &Dst))
    return std::make_pair(FgpuInstrInfo::BT_Uncond, BranchInstrs[1]);

  return std::make_pair(FgpuInstrInfo::BT_None, nullptr);
}

bool FgpuDelaySlotFiller::examinePred(MachineBasicBlock &Pred,
                                      const MachineBasicBlock &Succ,
                                      RegDefsUses &RegDU,
                                      bool &HasMultipleSuccs,
                                      BB2BrMap &BrMap) const {
  std::pair<FgpuInstrInfo::BranchType, MachineInstr *> P =
      getBranch(Pred, Succ);

  // Return if either getBranch wasn't able to analyze the branches or there
  // were no branches with unoccupied slots.
  if (P.first == FgpuInstrInfo::BT_None)
    return false;

  if ((P.first != FgpuInstrInfo::BT_Uncond) &&
      (P.first != FgpuInstrInfo::BT_NoBranch)) {
...
bool FgpuDelaySlotFiller::delayHasHazard(const MachineInstr &Candidate,
                                         RegDefsUses &RegDU,
                                         InspectMemInstr &IM) const {
...
bool FgpuDelaySlotFiller::terminateSearch(const MachineInstr &Candidate) const {
  return (Candidate.isTerminator() || Candidate.isCall() ||
          Candidate.isPosition() || Candidate.isInlineAsm() ||
          Candidate.hasUnmodeledSideEffects());
}

/// createFgpuDelaySlotFillerPass - Returns a pass that fills in delay
/// slots in Fgpu MachineFunctions
FunctionPass *llvm::createFgpuDelaySlotFillerPass() {
  return new FgpuDelaySlotFiller();
}
