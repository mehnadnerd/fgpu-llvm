...
/// section.
bool FgpuTargetObjectFile::IsGlobalInSmallSection(
...
IsGlobalInSmallSectionImpl(const GlobalObject *GO,
  ...
  const FgpuSubtarget &Subtarget =
      *static_cast<const FgpuTargetMachine &>(TM).getSubtargetImpl();
...
MCSection *FgpuTargetObjectFile::SelectSectionForGlobal(
...
bool FgpuTargetObjectFile::IsConstantInSmallSection(
...
/// Return true if this constant should be placed into small data section.
MCSection *FgpuTargetObjectFile::getSectionForConstant(const DataLayout &DL,
                                                       SectionKind Kind,
                                                       const Constant *C,
...
const MCExpr *
FgpuTargetObjectFile::getDebugThreadLocalSymbol(const MCSymbol *Sym) const {
  const MCExpr *Expr =
      MCSymbolRefExpr::create(Sym, MCSymbolRefExpr::VK_None, getContext());
  Expr = MCBinaryExpr::createAdd(
      Expr, MCConstantExpr::create(0x8000, getContext()), getContext());
  return FgpuMCExpr::create(FgpuMCExpr::MEK_DTPREL, Expr, getContext());
}
