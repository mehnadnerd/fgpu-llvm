...
static bool CC_FgpuO32_FP32(unsigned ValNo, MVT ValVT, MVT LocVT,
                            CCValAssign::LocInfo LocInfo,
                            ISD::ArgFlagsTy ArgFlags, CCState &State) {
  llvm_unreachable("should not be called");
}

static bool CC_FgpuO32_FP64(unsigned ValNo, MVT ValVT, MVT LocVT,
                            CCValAssign::LocInfo LocInfo,
                            ISD::ArgFlagsTy ArgFlags, CCState &State) {
  llvm_unreachable("should not be called");
}
...
unsigned FgpuFastISel::emitLogicalOp(unsigned ISDOpc, MVT RetVT,
  ...
  unsigned ResultReg = createResultReg(&Fgpu::GPR32RegClass);
  if (!ResultReg)
    return 0;
...
unsigned FgpuFastISel::materialize32BitInt(int64_t Imm,
  ...
  if (isInt<16>(Imm)) {
    unsigned Opc = Fgpu::ADDiu;
    emitInst(Opc, ResultReg).addReg(Fgpu::ZERO).addImm(Imm);
    return ResultReg;
  } else if (isUInt<16>(Imm)) {
    emitInst(Fgpu::ORi, ResultReg).addReg(Fgpu::ZERO).addImm(Imm);
    return ResultReg;
  }
  ...
  if (Lo) {
    ...
    emitInst(Fgpu::ORi, ResultReg).addReg(TmpReg).addImm(Lo);
  } else {
...
unsigned FgpuFastISel::materializeFP(const ConstantFP *CFP, MVT VT) {
  ...
  if (VT == MVT::f32) {
    ...
    unsigned TempReg = materialize32BitInt(Imm, &Fgpu::GPR32RegClass);
    emitInst(Fgpu::MTC1, DestReg).addReg(TempReg);
    return DestReg;
  } else if (VT == MVT::f64) {
    const TargetRegisterClass *RC = &Fgpu::AFGR64RegClass;
    unsigned DestReg = createResultReg(RC);
    unsigned TempReg1 = materialize32BitInt(Imm >> 32, &Fgpu::GPR32RegClass);
    unsigned TempReg2 =
        materialize32BitInt(Imm & 0xFFFFFFFF, &Fgpu::GPR32RegClass);
...
bool FgpuFastISel::emitCmp(unsigned ResultReg, const CmpInst *CI) {
  ...
  case CmpInst::ICMP_EQ: {
    unsigned TempReg = createResultReg(&Fgpu::GPR32RegClass);
  ...
  case CmpInst::FCMP_OGE: {
    ...
    switch (P) {
    case CmpInst::FCMP_OEQ:
      Opc = IsFloat ? Fgpu::C_EQ_S : Fgpu::C_EQ_D32;
      CondMovOpc = Fgpu::MOVT_I;
      break;
    case CmpInst::FCMP_UNE:
      Opc = IsFloat ? Fgpu::C_EQ_S : Fgpu::C_EQ_D32;
      CondMovOpc = Fgpu::MOVF_I;
      break;
    case CmpInst::FCMP_OLT:
      Opc = IsFloat ? Fgpu::C_OLT_S : Fgpu::C_OLT_D32;
      CondMovOpc = Fgpu::MOVT_I;
      break;
    case CmpInst::FCMP_OLE:
      Opc = IsFloat ? Fgpu::C_OLE_S : Fgpu::C_OLE_D32;
      CondMovOpc = Fgpu::MOVT_I;
      break;
    case CmpInst::FCMP_OGT:
      Opc = IsFloat ? Fgpu::C_ULE_S : Fgpu::C_ULE_D32;
      CondMovOpc = Fgpu::MOVF_I;
      break;
    case CmpInst::FCMP_OGE:
      Opc = IsFloat ? Fgpu::C_ULT_S : Fgpu::C_ULT_D32;
      CondMovOpc = Fgpu::MOVF_I;
      break;
    default:
      llvm_unreachable("Only switching of a subset of CCs.");
    }
    unsigned RegWithZero = createResultReg(&Fgpu::GPR32RegClass);
    unsigned RegWithOne = createResultReg(&Fgpu::GPR32RegClass);
...
bool FgpuFastISel::emitLoad(MVT VT, unsigned &ResultReg, Address &Addr,
  ...
  switch (VT.SimpleTy) {
  case MVT::i32:
    ResultReg = createResultReg(&Fgpu::GPR32RegClass);
    Opc = Fgpu::LW;
    break;
  case MVT::i16:
    ResultReg = createResultReg(&Fgpu::GPR32RegClass);
  ...
  case MVT::f32:
    if (UnsupportedFPMode)
      return false;
    ResultReg = createResultReg(&Fgpu::FGR32RegClass);
    Opc = Fgpu::LWC1;
    break;
  case MVT::f64:
    if (UnsupportedFPMode)
      return false;
    ResultReg = createResultReg(&Fgpu::AFGR64RegClass);
    Opc = Fgpu::LDC1;
    break;
  default:
    return false;
...
bool FgpuFastISel::selectBranch(const Instruction *I) {
  ...
  if (const CmpInst *CI = dyn_cast<CmpInst>(BI->getCondition())) {
    if (CI->hasOneUse() && CI->getParent() == I->getParent()) {
      ZExtCondReg = createResultReg(&Fgpu::GPR32RegClass);
...
bool FgpuFastISel::selectSelect(const Instruction *I) {
  ...
  unsigned ZExtCondReg = createResultReg(&Fgpu::GPR32RegClass);
  if (!ZExtCondReg)
    return false;
...
bool FgpuFastISel::selectFPToInt(const Instruction *I, bool IsSigned) {
  ...
  unsigned DestReg = createResultReg(&Fgpu::GPR32RegClass);
...
bool FgpuFastISel::fastLowerArguments() {
  ...
  for (const auto &FormalArg : F->args()) {
    ...
    case MVT::i16:
      ...
      Allocation.emplace_back(&Fgpu::GPR32RegClass, *NextGPR32++);
  ...
  case Intrinsic::bswap: {
    ...
    if (VT == MVT::i16) {
      if (Subtarget->hasFgpu32r2()) {
        ...
        unsigned TempReg[3];
        for (int i = 0; i < 3; i++) {
          TempReg[i] = createResultReg(&Fgpu::GPR32RegClass);
          if (TempReg[i] == 0)
            return false;
        }
...
bool FgpuFastISel::selectRet(const Instruction *I) {
  ...
  MachineInstrBuilder MIB = emitInst(Fgpu::RetRA);
...
void FgpuFastISel::simplifyAddress(Address &Addr) {
  if (!isInt<16>(Addr.getOffset())) {
    unsigned TempReg =
        materialize32BitInt(Addr.getOffset(), &Fgpu::GPR32RegClass);
    unsigned DestReg = createResultReg(&Fgpu::GPR32RegClass);
